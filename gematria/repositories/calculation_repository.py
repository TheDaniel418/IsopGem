...
class CalculationRepository:
    ...
    def __init__(self, data_dir: Optional[str] = None) -> None:
        ...
        logger.debug(f"CalculationRepository initialized with data directory: {self._data_dir}")
    ...
    def find_by_text(self, text: str) -> List[CalculationResult]:
        ...
        return [calc for calc in all_calculations if text.lower() in calc.input_text.lower()]
    ...
    def find_by_method(self, method: Union[str, int]) -> List[CalculationResult]:
        ...
        return [
            ...
            if (
                ...
                (isinstance(calc.calculation_type, str) and calc.calculation_type == method_str)
                or 
                # Handle CalculationType objects
                (hasattr(calc.calculation_type, 'value') and str(calc.calculation_type.value) == method_str)
            )
        ]
    ...
    def find_calculations_by_text(self, text: str) -> List[CalculationResult]:
        ...
        return [
            calc for calc in self.get_all_calculations() 
            if text_lower in calc.input_text.lower() or 
               (calc.notes and text_lower in calc.notes.lower())
        ]
