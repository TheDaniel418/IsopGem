...
from PyQt6.QtWidgets import (
    ...
    QTextEdit, QCheckBox, QGroupBox, QWidget, QMessageBox,
...
from gematria.ui.dialogs.tag_selection_dialog import ColorSquare
...
class SaveCalculationDialog(QDialog):
    ...
    def _init_ui(self):
        ...
        self.notes_edit.setPlaceholderText("Enter notes about this calculation (optional)")
    ...
    def _select_tags(self):
        ...
        for tag in all_tags:
            ...
            checkbox.stateChanged.connect(
                lambda state, tid=tag.id: 
                selected_tags.add(tid) if state == Qt.CheckState.Checked.value 
                else selected_tags.discard(tid)
            )
        
        # Button to create new tag
        create_btn = QPushButton("Create New Tag")
        create_btn.clicked.connect(lambda: self._create_tag_from_dialog(dialog, tag_list, selected_tags))
