...
class ImportWordListDialog(QDialog):
    ...
    def _setup_ui(self) -> None:
        ...
        self._calc_all_methods_checkbox = QCheckBox("Calculate using all available methods")
    ...
    def _load_excel_sheets(self) -> None:
        ...
        try:
            ...
            # Select first sheet
            if xl.sheet_names:
                self._selected_sheet = xl.sheet_names[0]
                self._load_data()
            ...
            QMessageBox.critical(
                self,
                "Error Loading File",
                f"Could not load Excel sheets: {str(e)}"
            )
    ...
    def _update_column_combos(self) -> None:
        ...
        self._word_column_combo.addItems(columns)
        self._notes_column_combo.addItems([''] + columns)
        self._tags_column_combo.addItems([''] + columns)
        ...
        self._update_column_mapping("word", self._word_column_combo.currentText())
        self._update_column_mapping("notes", self._notes_column_combo.currentText())
        self._update_column_mapping("tags", self._tags_column_combo.currentText())
    ...
    def _import_data(self) -> None:
        ...
        self._progress_bar.setMaximum(len(self._dataframe))
