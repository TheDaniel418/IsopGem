...
class CreateTagDialog(QDialog):
    ...
    def _init_ui(self):
        ...
        self.color_preview.setStyleSheet(
            f"background-color: {self._selected_color}; border: 1px solid #ccc;"
        )
    ...
    def _choose_color(self):
        ...
        color = QColorDialog.getColor(
            QColor(self._selected_color), self, "Choose Tag Color"
        )
    ...
    def _create_tag(self):
        ...
        try:
            tag = self.db_service.create_tag(
                name=name, color=self._selected_color, description=description
            )
            ...
            if tag:
                ...
                QMessageBox.warning(
                    self, "Error", "Failed to create tag. Please try again."
                )

        except Exception as e:
            logger.error(f"Error creating tag: {e}")
            QMessageBox.critical(
                self, "Error", f"An error occurred while creating the tag: {e}"
            )
