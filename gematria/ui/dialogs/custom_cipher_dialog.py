...
from typing import Dict, Optional, List, Any, Union, Set, cast, TypeVar, Generic, Callable
from PyQt6.QtWidgets import (
    QDialog, QVBoxLayout, QHBoxLayout, QFormLayout, QGroupBox, 
    QLabel, QLineEdit, QTextEdit, QComboBox, QCheckBox, 
    QPushButton, QScrollArea, QWidget, QMessageBox, QGridLayout,
    QTableWidget, QTableWidgetItem, QSpinBox, QTabWidget, QListWidget, QListWidgetItem
)
...
class CustomCipherDialog(QDialog):
    ...
    def _cipher_selected(self) -> None:
        ...
        cipher_id = self.cipher_table.item(selected_rows[0].row(), 0).data(Qt.ItemDataRole.UserRole)
    ...
    def _update_cipher_from_editor(self) -> bool:
        ...
        if not letter_values:
            QMessageBox.warning(
                self, 
                "Validation Error", 
                "You must assign at least one letter value"
            )
            return False
    ...
    def _save_current_cipher(self) -> None:
        ...
        if success:
            ...
            QMessageBox.warning(
                self, 
                "Save Error", 
                "Failed to save the cipher configuration"
            )
    ...
    def _delete_selected_cipher(self) -> None:
        ...
        if not selected_rows:
            QMessageBox.warning(
                self, 
                "No Selection", 
                "Please select a cipher to delete"
            )
            return
        ...
        if self.service.delete_cipher(cipher_id):
            ...
            QMessageBox.information(
                self, 
                "Success", 
                f"Cipher '{cipher_name}' deleted successfully"
            )
        else:
            QMessageBox.warning(
                self, 
                "Delete Error", 
                f"Failed to delete the cipher '{cipher_name}'"
            )
...
class CipherSelectionDialog(QDialog):
    ...
    def _create_custom_cipher_from_calc_type(self, calc_type: CalculationType, name: str, language: LanguageType) -> Optional[CustomCipherConfig]:
        ...
        hebrew_reversal = {
            'א': 400, 'ב': 300, 'ג': 200, 'ד': 100, 'ה': 90, 'ו': 80, 'ז': 70, 'ח': 60, 'ט': 50,
            'י': 40, 'כ': 30, 'ל': 20, 'מ': 10, 'נ': 9, 'ס': 8, 'ע': 7, 'פ': 6, 'צ': 5,
            'ק': 4, 'ר': 3, 'ש': 2, 'ת': 1,
        ...
        # Albam substitution
        hebrew_albam = {
            'א': 30, 'ב': 40, 'ג': 50, 'ד': 60, 'ה': 70, 'ו': 80, 'ז': 90, 'ח': 100, 'ט': 200,
            'י': 300, 'כ': 400, 'ל': 1, 'מ': 2, 'נ': 3, 'ס': 4, 'ע': 5, 'פ': 6, 'צ': 7,
            'ק': 8, 'ר': 9, 'ש': 10, 'ת': 20,
            'ך': 400, 'ם': 2, 'ן': 3, 'ף': 6, 'ץ': 7
        }
        ...
        greek_ordinal = {
            ...
            'ρ': 18, 'σ': 19, 'τ': 20, 'υ': 21, 'φ': 22, 'χ': 23, 'ψ': 24, 'ω': 25,
            'ς': 19
        }
        ...
        greek_reduced = {
            'α': 1, 'β': 2, 'γ': 3, 'δ': 4, 'ε': 5, 'ζ': 7, 'η': 8, 'θ': 9,
            'ι': 1, 'κ': 2, 'λ': 3, 'μ': 4, 'ν': 5, 'ξ': 6, 'ο': 7, 'π': 8,
            'ρ': 1, 'σ': 2, 'τ': 3, 'υ': 4, 'φ': 5, 'χ': 6, 'ψ': 7, 'ω': 8,
            'ς': 2
        }
        
        # Greek reversal values
        greek_reversal = {
            'α': 800, 'β': 700, 'γ': 600, 'δ': 500, 'ε': 400, 'ζ': 300, 'η': 200, 'θ': 100,
            'ι': 80, 'κ': 70, 'λ': 60, 'μ': 50, 'ν': 40, 'ξ': 30, 'ο': 20, 'π': 10,
            'ρ': 9, 'σ': 8, 'τ': 7, 'υ': 6, 'φ': 5, 'χ': 4, 'ψ': 3, 'ω': 1,
            'ς': 8
        }
        
        # Greek Alpha-Mu cipher (similar to Hebrew Albam)
        greek_alpha_mu = {
            'α': 40, 'β': 50, 'γ': 60, 'δ': 70, 'ε': 80, 'ζ': 90, 'η': 100, 'θ': 200,
            'ι': 300, 'κ': 400, 'λ': 500, 'μ': 600, 'ν': 1, 'ξ': 2, 'ο': 3, 'π': 4,
            'ρ': 5, 'σ': 6, 'τ': 7, 'υ': 8, 'φ': 9, 'χ': 10, 'ψ': 20, 'ω': 30,
            'ς': 6
        }
        ...
        # TQ English values
        english_tq = {
            'i': 0, 'I': 0, 'l': 1, 'L': 1, 'c': 2, 'C': 2, 'h': 3, 'H': 3,
            'p': 4, 'P': 4, 'a': 5, 'A': 5, 'x': 6, 'X': 6, 'j': 7, 'J': 7,
            'w': 8, 'W': 8, 't': 9, 'T': 9, 'o': 10, 'O': 10, 'g': 11, 'G': 11,
            'f': 12, 'F': 12, 'e': 13, 'E': 13, 'r': 14, 'R': 14, 's': 15, 'S': 15,
            'q': 16, 'Q': 16, 'k': 17, 'K': 17, 'y': 18, 'Y': 18, 'z': 19, 'Z': 19,
            'b': 20, 'B': 20, 'm': 21, 'M': 21, 'v': 22, 'V': 22, 'd': 23, 'D': 23,
            'n': 24, 'N': 24, 'u': 25, 'U': 25
        }
    ...
    def _populate_method_categories(self) -> None:
        ...
        if language == LanguageType.HEBREW:
            ...
            if hebrew_custom_ciphers:
                hebrew_categories["Custom Methods"] = cast(List[MethodType], hebrew_custom_ciphers)
            ...
            if greek_custom_ciphers:
                greek_categories["Custom Methods"] = cast(List[MethodType], greek_custom_ciphers)
            ...
            english_categories: Dict[str, List[MethodType]] = {
                "TQ Methods": [
                    CalculationType.TQ_ENGLISH
                ]
            }
            
            # Add custom ciphers category if available
            english_custom_ciphers = self.custom_service.get_ciphers(LanguageType.ENGLISH)
            if english_custom_ciphers:
                english_categories["Custom Methods"] = cast(List[MethodType], english_custom_ciphers)
        ...
        for method in methods:
            if isinstance(method, CalculationType):
                ...
                if method.description:
                    ...
                    item.setToolTip(f"Custom {method.language.name.capitalize()} cipher")
