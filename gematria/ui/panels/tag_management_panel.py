...
from PyQt6.QtWidgets import (
    ...
    QFormLayout, QLineEdit, QTextEdit, QComboBox, QFrame,
...
class TagManagementPanel(QWidget):
    ...
    def _update_stats(self):
        ...
        self.stats_label.setText(f"{len(tags)} tags defined, used in {total_tag_usages} calculations")
        ...
        if confirm == QMessageBox.StandardButton.Yes:
            if self.db_service.delete_tag(tag.id):
                ...
                QMessageBox.warning(
                    self,
                    "Error",
                    f"Failed to delete tag '{tag.name}'"
                ) 