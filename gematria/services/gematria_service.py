...
class GematriaService:
    ...
    def __init__(self) -> None:
        ...
        self._greek_values = {
            ...
            'ϙ': 90, 'ρ': 100, 'σ': 200, 'τ': 300, 'υ': 400, 'φ': 500, 'χ': 600, 'ψ': 700, 'ω': 800,
            # Variant forms
            'ς': 200, 'ϲ': 200  # Final sigma variants
        }
        ...
        self._greek_positions = {
            ...
            'ϙ': 18, 'ρ': 19, 'σ': 20, 'τ': 21, 'υ': 22, 'φ': 23, 'χ': 24, 'ψ': 25, 'ω': 26,
            'ς': 20, 'ϲ': 20  # Final sigma variants
        }

        # Greek alpha-mu cipher (similar to Hebrew Albam)
        self._greek_alpha_mu_map = {
            # First half of alphabet maps to second half
            'α': 'ν', 'β': 'ξ', 'γ': 'ο', 'δ': 'π', 'ε': 'ϙ', 'ϝ': 'ρ', 
            'ζ': 'σ', 'η': 'τ', 'θ': 'υ', 'ι': 'φ', 'κ': 'χ', 'λ': 'ψ', 'μ': 'ω',
            # Second half of alphabet maps to first half
            'ν': 'α', 'ξ': 'β', 'ο': 'γ', 'π': 'δ', 'ϙ': 'ε', 'ρ': 'ϝ', 
            'σ': 'ζ', 'τ': 'η', 'υ': 'θ', 'φ': 'ι', 'χ': 'κ', 'ψ': 'λ', 'ω': 'μ',
            # Variant forms
            'ς': 'ζ', 'ϲ': 'ζ'
        }
        ...
        self._greek_alpha_omega_map = {
            ...
            'α': 'ω', 'β': 'ψ', 'γ': 'χ', 'δ': 'φ', 'ε': 'υ', 'ϝ': 'τ', 'ζ': 'σ',
            'η': 'ρ', 'θ': 'ϙ', 'ι': 'π', 'κ': 'ο', 'λ': 'ξ', 'μ': 'ν',
            'ν': 'μ', 'ξ': 'λ', 'ο': 'κ', 'π': 'ι', 'ϙ': 'θ', 'ρ': 'η', 'σ': 'ζ',
            'τ': 'ϝ', 'υ': 'ε', 'φ': 'δ', 'χ': 'γ', 'ψ': 'β', 'ω': 'α',
            # Variant forms
            'ς': 'ζ', 'ϲ': 'ζ'
        }
        ...
        self._albam_map = {
            ...
            # Second half of alphabet maps to first half
            'ל': 'א', 'מ': 'ב', 'נ': 'ג', 'ס': 'ד', 'ע': 'ה', 'פ': 'ו', 'צ': 'ז', 'ק': 'ח', 'ר': 'ט', 'ש': 'י', 'ת': 'כ',
            # Final forms
            'ך': 'ת', 'ם': 'ב', 'ן': 'ג', 'ף': 'ו', 'ץ': 'ז'
        }
    ...
    def calculate(self, 
                 text: str, 
                 calculation_type: Union[CalculationType, str, CustomCipherConfig] = CalculationType.MISPAR_HECHRACHI) -> int:
    ...
    def _strip_diacritical_marks(self, text: str) -> str:
        ...
        clean_text = ''.join(c for c in normalized if not unicodedata.category(c).startswith('M'))
    ...
    def _calculate_triangular(self, text: str) -> int:
        ...
        triangular_values = {
            'א': 1, 'ב': 3, 'ג': 6, 'ד': 10, 'ה': 15, 'ו': 21, 'ז': 28, 'ח': 36, 'ט': 45,
            'י': 55, 'כ': 75, 'ל': 105, 'מ': 145, 'נ': 195, 'ס': 255, 'ע': 325, 'פ': 405, 'צ': 495,
    ...
    def _calculate_greek_reversal(self, text: str) -> int:
        ...
        reversal_values = {
            'α': 800, 'β': 700, 'γ': 600, 'δ': 500, 'ε': 400, 'ϝ': 300, 'ζ': 200, 
            'η': 100, 'θ': 90, 'ι': 80, 'κ': 70, 'λ': 60, 'μ': 50, 'ν': 40, 
            'ξ': 30, 'ο': 20, 'π': 10, 'ϙ': 9, 'ρ': 8, 'σ': 7, 'τ': 6, 
            'υ': 5, 'φ': 4, 'χ': 3, 'ψ': 2, 'ω': 1,
            # Variant forms
            'ς': 7, 'ϲ': 7
        }
    ...
    _greek_letter_names = {
        ...
        'β': {'name': 'βῆτα', 'value': 308},     # beta
        'γ': {'name': 'γάμμα', 'value': 145},    # gamma
        'δ': {'name': 'δέλτα', 'value': 339},    # delta
        'ε': {'name': 'ἒψιλον', 'value': 658},   # epsilon
        'ϝ': {'name': 'δίγαμμα', 'value': 449},  # digamma
        'ζ': {'name': 'ζῆτα', 'value': 308},     # zeta
        'η': {'name': 'ἦτα', 'value': 314},      # eta
        'θ': {'name': 'θῆτα', 'value': 318},     # theta
        'ι': {'name': 'ἰῶτα', 'value': 811},     # iota
        'κ': {'name': 'κάππα', 'value': 322},    # kappa
        'λ': {'name': 'λάμβδα', 'value': 378},   # lambda
        'μ': {'name': 'μῦ', 'value': 440},       # mu
        'ν': {'name': 'νῦ', 'value': 450},       # nu
        'ξ': {'name': 'ξῖ', 'value': 70},        # xi
        'ο': {'name': 'ὂμικρον', 'value': 430},  # omicron
        'π': {'name': 'πῖ', 'value': 90},        # pi
        'ϙ': {'name': 'κόππα', 'value': 391},    # koppa
        'ρ': {'name': 'ῥῶ', 'value': 1100},      # rho
        'σ': {'name': 'σίγμα', 'value': 374},    # sigma
        'τ': {'name': 'ταῦ', 'value': 701},      # tau
        'υ': {'name': 'ὒψιλον', 'value': 1350},  # upsilon
        'φ': {'name': 'φῖ', 'value': 510},       # phi
        'χ': {'name': 'χῖ', 'value': 610},       # chi
        'ψ': {'name': 'ψῖ', 'value': 710},       # psi
        'ω': {'name': 'ὦμέγα', 'value': 1371},   # omega
        'ς': {'name': 'σίγμα τελικόν', 'value': 374}, # final sigma
        'ϲ': {'name': 'σίγμα τελικόν', 'value': 374}  # final sigma variant
    }
    ...
    _greek_triangular_values = {
        ...
        'ι': 55, 'κ': 75, 'λ': 105, 'μ': 145, 'ν': 195, 'ξ': 255, 'ο': 325, 'π': 405,
        'ϙ': 495, 'ρ': 595, 'σ': 795, 'τ': 1095, 'υ': 1495, 'φ': 1995, 'χ': 2595, 'ψ': 3295, 'ω': 4095,
        # Variant forms
        'ς': 795, 'ϲ': 795
    }
    ...
    def calculate_and_save(self, 
                          text: str, 
                          calculation_type: Union[CalculationType, str, CustomCipherConfig] = CalculationType.MISPAR_HECHRACHI,
                          notes: Optional[str] = None,
                          tags: Optional[List[str]] = None,
                          favorite: bool = False) -> CalculationResult:
