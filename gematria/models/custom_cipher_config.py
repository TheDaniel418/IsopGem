...
class CustomCipherConfig(BaseModel):
    ...
    id: str = Field(
        default="", description="Unique identifier for the cipher configuration"
    )
    name: str = Field(description="Display name for the cipher")
    language: LanguageType = Field(description="Language this cipher applies to")
    description: str = Field(
        default="", description="Description of the cipher and its significance"
    )
    letter_values: Dict[str, int] = Field(
        default_factory=dict, description="Mapping of letters to their numerical values"
    )
    case_sensitive: bool = Field(
        default=False, description="Whether letter case matters for this cipher"
    )
    ...
    def __init__(
        self, name: str, language: LanguageType, description: str = ""
    ) -> None:
    ...
    def is_valid(self) -> bool:
        ...
        if self.language == LanguageType.HEBREW:
            ...
            if (
                len(found_lower) < len(english_primary) / 2
                and len(found_upper) < len(english_upper) / 2
            ):
                return False
