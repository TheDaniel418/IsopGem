...
class CosmicForceInterpreter:
    ...
    def interpret_digit(self, digit: int, position: int) -> Dict:
        ...
        force_info = self.FORCE_MEANINGS.get(
            digit, {"name": "Unknown", "description": "Unknown force"}
        )
        ...
        if digit == 0:
            ...
            if position > 7:
                ...
                specific += (
                    "This grounds the system with balance at the foundational level."
                )
        elif digit == 1:
            ...
            if position > 7:
                ...
                specific += (
                    "This gathers and concentrates energy at the foundational level."
                )
    ...
    def analyze_pattern(self, ternary_str: str) -> Dict:
        ...
        decimal_value = sum(
            int(digit) * (3**i) for i, digit in enumerate(reversed(ternary_str))
        )
    ...
    def _generate_pattern_text(self, ternary_str: str, analysis: Dict) -> str:
        ...
        raw_ratio = sum(int(d) for d in ternary_str) / (
            total_digits * 2
        )  # Normalize to 0-1 range
        base_frequency = 20 * (
            1000**raw_ratio
        )  # Exponential scaling from 20 Hz to 20 kHz
        ...
        # Harmonic structure
        harmonic_index = (
            counts[1] * 1.5 + counts[2] * 2.0 + counts[0] * 0.5
        ) / total_digits
        ...
        pattern_complexity = (
            unique_pairs / len(digit_pairs) if len(digit_pairs) > 0 else 0
        )
        ...
        transition_rate = (
            transitions / (len(ternary_str) - 1) if len(ternary_str) > 1 else 0
        )
        ...
        sequence_uniqueness = (
            len(set(ternary_str)) / 3.0
        )  # How many of the possible digits are used
        sequence_pattern = ""
        ...
        for pattern_length in range(1, len(ternary_str) // 2 + 1):
            ...
            if (
                pattern_count > 1
                and pattern_count * pattern_length > len(ternary_str) * 0.5
            ):
        ...
        if len(ternary_str) >= 5:
            ...
            if len(runs) >= 3:
                ...
                for i in range(2, len(runs)):
                    if (
                        abs(runs[i] - (runs[i - 1] + runs[i - 2])) <= 1
                    ):  # Allow small variance
                        fib_like += 1

                if (
                    fib_like >= (len(runs) - 2) * 0.5
                ):  # If at least half are Fibonacci-like
                    fib_present = True
        ...
        for change in force_succession:
            if change == 0:
                ...
                1,
                -2,
            ]:  # Increasing force (0->1, 1->2, or 2->0 with wraparound)
        ...
        angular_momentum = (
            (clockwise_count - counterclockwise_count) / len(directions)
            if len(directions) > 0
            else 0
        )
        ...
        if len(ternary_str) >= 4:
            ...
            for i in range(len(ternary_str) // 2):
                if ternary_str[i] == ternary_str[-(i + 1)]:
                    symmetry_score += 1
            symmetry_score = (
                symmetry_score / (len(ternary_str) // 2)
                if len(ternary_str) // 2 > 0
                else 0
            )
        ...
        if analysis["digital_root"] == 3:
            text += (
                "representing the trinity principle and fundamental creative forces."
            )
            ...
            text += (
                "representing cosmic completion and the culmination of a force cycle."
            )
        else:
    ...
    def _display_position_analysis(self, ternary_str: str):
        ...
        for position in range(len(ternary_str), 0, -1):
            ...
            if digit == 0:
                ...
                )  # Light orange for Expansive
            else:  # digit == 2
                position_frame.setStyleSheet(
                    "background-color: #E6FFE6; border: 1px solid #99FF99;"
                )  # Light green for Contractive

            frame_layout = QVBoxLayout(position_frame)

            # Position header
            header = QLabel(
                f"Position {position}: {interpretation['position_name']} (3^{position-1} = {interpretation['position_value']})"
            )
