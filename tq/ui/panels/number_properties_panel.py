...
class PropertiesTab(QWidget):
        ...
        try:
            ...
            if results:
                ...
                for result in results:
                    ...
                    results_by_type[calc_type].append(result.input_text)  # Use input_text instead of text
                ...
                self.results_label.setText(f"No words found with value {self.current_number}.")
    ...
    def _send_to_transitions(self):
        ...
        try:
            ...
            abundance_text = self.special.last_properties.get("Abundance/Deficiency", "0")
            ...
            logger.debug(f"Sending to transitions - aliquot_sum: {aliquot_sum}, abundance_value: {abundance_value}")
            ...
            QMessageBox.warning(self, "Error", f"Could not send values to Ternary Transitions: {str(e)}")
    ...
    def _send_to_series_transitions(self):
        ...
        try:
            ...
            for i, (first, second) in enumerate(pairs):
                ...
                window.transition_widget.pair_inputs[i].second_number.setText(str(second))
            ...
            from PyQt6.QtWidgets import QMessageBox
            QMessageBox.warning(self, "Error", f"Could not send factors to Series Transitions: {str(e)}")
    ...
    def update_properties(self, properties: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if factors:
                ...
                self.series_button.setEnabled(True)  # Enable the button if we have factors
            ...
            abundance_text = properties.get("special", {}).get("Abundance/Deficiency", "")
            is_perfect = "perfect" in abundance_text.lower()
            self.transition_button.setEnabled(not is_perfect)  # Enable only if not perfect
