...
class NumberPairWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.grid_button.clicked.connect(lambda: self._open_in_tq_grid(self.result_decimal.text()))
        self.lookup_button.clicked.connect(lambda: self._lookup_in_database(self.result_decimal.text()))
...
class SeriesTransitionWidget(QWidget):
    ...
    def _init_ui(self):
        ...
        self.series_sum_ternary.setStyleSheet("font-family: monospace; font-size: 16px; font-weight: bold;")
        ...
        self.closed_loop_ternary.setStyleSheet("font-family: monospace; font-size: 16px; font-weight: bold;")
        ...
        self.amalgam_ternary.setStyleSheet("font-family: monospace; font-size: 16px; font-weight: bold;")
        ...
        self.series_sum_grid.clicked.connect(lambda: self._open_in_tq_grid(self.series_sum_decimal.text()))
        self.series_sum_lookup.clicked.connect(lambda: self._lookup_in_database(self.series_sum_decimal.text()))
        self.closed_loop_grid.clicked.connect(lambda: self._open_in_tq_grid(self.closed_loop_decimal.text()))
        self.closed_loop_lookup.clicked.connect(lambda: self._lookup_in_database(self.closed_loop_decimal.text()))
        self.amalgam_grid.clicked.connect(lambda: self._open_in_tq_grid(self.amalgam_decimal.text()))
        self.amalgam_lookup.clicked.connect(lambda: self._lookup_in_database(self.amalgam_decimal.text()))
    ...
    def _calculate_transitions(self):
        ...
        try:
            ...
            for i, (decimal_result, ternary_result) in enumerate(zip(result.transitions, result.transition_ternaries)):
            ...
            QMessageBox.warning(self, "Error", f"Could not calculate transitions: {str(e)}")
