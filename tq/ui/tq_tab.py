...
from tq.ui.widgets.tq_grid_panel import TQGridPanel
...
class TernaryDigit:
    ...
    def __init__(self):
        ...
        self.dy = random.uniform(0.2, 0.6)  # Slightly slower for better trail visibility
        ...
        for i in range(self.tail_length):
            ...
            self.segments.append({
                'value': random.randint(0, 2),  # Each segment can be a different value
                'y_offset': -i * (self.size * 0.4),  # Space segments vertically (closer together)
                # Slower fade-out with longer visible trail
                'alpha': max(0.0, min(1.0, 1.0 - (i * 0.5 / self.tail_length)))  # More gradual alpha reduction
            })
        ...
        self.brightness_factor = 0.7 + (self.value * 0.1)  # Reduced from 0.15 to 0.1 to stay < 1.0
    ...
    def update_animation(self):
        ...
        if hasattr(self, '_debug_counter'):
            ...
            if self._debug_counter >= 60:  # Log every ~60 frames
                ...
                logger.debug(f"MatrixCanvas animation update: {self.width()}x{self.height()}, visible: {self.isVisible()}")
        else:
            self._debug_counter = 0
    ...
    def paintEvent(self, event):
        ...
        for digit in self.digits:
            ...
            for segment in reversed(digit.segments):
                ...
                painter.drawText(
                    QRectF(x - digit.size/2, segment_y - digit.size/2, digit.size, digit.size),
            ...
            brightness = max(0.0, min(1.0, digit.value_prop * digit.brightness_factor * (0.8 + 0.2 * glow_factor)))
            main_color.setHsvF(
                max(0.0, min(1.0, digit.hue)),
...
class TQTab(QWidget):
    ...
    def _init_ui(self) -> None:
        ...
        # chart_btn.clicked.connect(lambda: self._open_chart_creator())
        ...
        if not leo_pixmap.isNull():
            ...
            caption.setStyleSheet("font-size: 12px; color: #00ff00; font-style: italic;")
            caption.setAlignment(Qt.AlignmentFlag.AlignCenter)
            
            leo_layout.addWidget(leo_image_label, alignment=Qt.AlignmentFlag.AlignCenter)
            leo_layout.addWidget(caption, alignment=Qt.AlignmentFlag.AlignCenter)
            
            # Add the image container to the content layout
            content_layout.addWidget(leo_container, alignment=Qt.AlignmentFlag.AlignCenter)
        else:
    ...
    def _open_tq_grid(self) -> None:
        ...
        tq_analysis_service.get_instance().open_quadset_analysis(0)  # Start with 0 as default
