...
from shared.services.calculation_database_service import CalculationDatabaseService
...
class TQDatabaseService(QObject):
    ...
    def _get_number_properties(self, number: int) -> Dict[str, Any]:
        ...
        if properties["is_triangular"]:
            properties["triangular_index"] = self.number_properties.get_triangular_index(number)
        
        if properties["is_fibonacci"]:
            properties["fibonacci_index"] = self.number_properties.get_fibonacci_index(number)
        ...
        try:
            ...
            for calc in calculations:
                references.append({
                    ...
                    "method": str(calc.calculation_type) if hasattr(calc, "calculation_type") else calc.method_name,
                    "tags": self.calculation_db.get_calculation_tag_names(calc) if hasattr(self.calculation_db, "get_calculation_tag_names") else [],
    ...
    def _check_if_in_quadset(self, number: int) -> Dict[str, Any]:
        ...
        try:
            ...
            if 'base' in quadset and isinstance(quadset['base'], dict):
                ...
                if (number == quadset['base'].get('number') or
                    number == quadset['conrune'].get('number') or
                    number == quadset['ternary_reversal'].get('number') or
                    number == quadset['reversal_conrune'].get('number')):
                ...
                upper_bound = min(number * 10, 10000)  # Limit the search to avoid performance issues
                ...
                for base in range(lower_bound, upper_bound):
                    ...
                    if 'base' in base_quadset and isinstance(base_quadset['base'], dict):
                        if (number == base_quadset['conrune'].get('number') or
                            number == base_quadset['ternary_reversal'].get('number') or
                            number == base_quadset['reversal_conrune'].get('number')):
            ...
            # Handle the flat dictionary structure (when quadset has 'number', 'conrune', etc. keys)
                ...
                if (number == quadset.get('number') or
                    number == quadset.get('conrune') or
                    number == quadset.get('reverse_ternary_decimal') or
                    number == quadset.get('reverse_conrune')):
                ...
                for base in range(lower_bound, upper_bound):
                    ...
                    base_quadset = self.number_properties.get_quadset_properties(base)
                    if 'number' in base_quadset and not isinstance(base_quadset.get('conrune'), dict):
                        if (number == base_quadset.get('conrune') or
                            number == base_quadset.get('reverse_ternary_decimal') or
                            number == base_quadset.get('reverse_conrune')):
