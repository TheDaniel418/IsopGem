...
def recover_from_error(parent: QObject, error: Exception, context: str = "") -> bool:
    ...
    try:
        ...
        logger.error(f"Attempting to recover from {error_type}: {error_msg} in {context}")
        ...
        # Generic recovery
            ...
            logger.warning(f"No specific recovery strategy for {error_type}, using generic recovery")
...
"""
        ...
        report_file = reports_dir / f"error_{timestamp.replace(':', '-').replace(' ', '_')}.txt"
