...
def handle_error(parent, error_message, exception=None, show_dialog=True, log_level="error"):
    """
    ...
    if hasattr(parent, "error_occurred") and callable(getattr(parent, "error_occurred", None)):
...
def handle_warning(parent, warning_message, show_dialog=True):
    ...
    # Emit signal if parent has one
    if hasattr(parent, "status_updated") and callable(getattr(parent, "status_updated", None)):
