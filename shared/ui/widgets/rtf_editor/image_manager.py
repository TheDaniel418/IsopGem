...
from shared.ui.widgets.rtf_editor.utils.recovery_utils import recover_from_error, create_error_report
from shared.ui.widgets.rtf_editor.utils.image_storage import (
    save_image_to_storage,
    data_uri_to_image_file,
    get_image_dimensions,
    cleanup_unused_images,
    export_document_with_images
)
...
class ImageManager(QObject):
    ...
    def insert_image(self):
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Error saving to storage, using original path: {str(e)}")
            ...
            if hasattr(self.parent_window, "image_manager_insert_image"):
                ...
                success = self.parent_window.image_manager_insert_image(image_format, position)
                if not success:
            ...
            for scale_factor in [0.5, 0.25, 0.1, 0.05]:
                try:
                    ...
                    with Image.open(path_obj) as img:
                        ...
                        logger.info(f"Successfully downsampled image to {new_width}x{new_height}")
                        ...
                        self.insert_image_at_cursor(str(temp_file), new_width, new_height)
                        return True
                        
                except MemoryError:
                    # If this scale factor still causes memory error, try a smaller one
                    logger.warning(f"Downsampling to {scale_factor} still caused memory error, trying smaller scale")
                    continue
    ...
    def convert_data_uris_to_files(self):
        ...
        try:
                ...
                if char_format.isImageFormat():
                    ...
                    if image_source.startswith("data:image/"):
                        try:
                            ...
                            logger.error(f"Error converting data URI: {str(e)}", exc_info=True)
            ...
            logger.error(f"Error converting data URIs to files: {str(e)}", exc_info=True)
            return 0
