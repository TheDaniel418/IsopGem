...
def run_test():
    ...
    assert len(window_manager._auxiliary_windows) == 0, "Windows from previous test weren't cleaned up"
    ...
    # Final summary
    all_tests_passed = (
        all([test1_success, test1_panels, test1_windows]) and
        test2_panel_count == 0 and test2_window_count == 3 and
        test3_panel_count == 0 and test3_window_count == 1
    )
    
    print("\n✨ TEST SUMMARY")
    if all_tests_passed:
        print("✅ ALL TESTS PASSED: Application is correctly using only free-floating windows")
    else:
        print("❌ SOME TESTS FAILED: Application may not be correctly configured for free-floating windows only")
