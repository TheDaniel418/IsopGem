...
def parse_arguments():
    ...
    """
    parser = argparse.ArgumentParser(description="Populate the astrological events database with aspects")
    
    parser.add_argument("--start-year", type=int, required=True,
                        help="Starting year for calculations")
    parser.add_argument("--end-year", type=int, required=True,
                        help="Ending year for calculations")
    parser.add_argument("--include-minor", action="store_true", default=False,
                       help="Include minor aspects (default: False, major aspects only)")
    parser.add_argument("--orb", type=float, default=1.0,
...
def main():
    ...
    logger.info(f"Starting aspect database population for years {args.start_year}-{args.end_year}")
    logger.info(f"Including {'major and minor' if args.include_minor else 'only major'} aspects with orb {args.orb}°")
    
    # Confirm with the user
    total_years = args.end_year - args.start_year + 1
    total_days = total_years * 365.25  # Approximate days
    print(f"This will calculate aspects for {total_years} years ({int(total_days)} days).")
    print(f"This operation may take a long time depending on the range.")
    ...
    try:
        ...
        result = db.query_one("SELECT name FROM sqlite_master WHERE type='table' AND name='celestial_bodies'")
        if not result:
        ...
        print(f"Starting aspect calculation for years {args.start_year}-{args.end_year}...")
