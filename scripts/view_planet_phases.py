...
def parse_args():
    ...
    date_group.add_argument(
        "--date", type=str, help="Specific date to view (YYYY-MM-DD)"
    )

    date_group.add_argument("--month", type=str, help="Month to view (YYYY-MM)")

    date_group.add_argument("--year", type=int, help="Year to view")

    # Content filtering arguments
    filter_group = parser.add_argument_group("Content filtering")
    filter_group.add_argument(
        "--planet", type=str, choices=["Mercury", "Venus"], help="Filter by planet"
    )
...
def format_output(phases: List[Dict[str, Any]], format_type: str) -> str:
    ...
    if format_type == "table":
        ...
        for phase in phases:
            ...
            # Format position
            position = (
                f"{phase['position_degree']:.1f}° {phase['zodiac_sign']}"
                if phase["position_degree"]
                else ""
            )

            # Format elongation
            elongation = (
                f"{phase['elongation_degree']:.1f}°"
                if phase["elongation_degree"]
                else ""
            )
            ...
            table_data.append(
                [date_str, phase["body_name"], phase_type, position, elongation]
            )
        ...
        for phase in phases:
            ...
            lines.append(
                f"{date_str},{phase['body_name']},{phase_type},{position},{elongation}"
            )
