...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "input_file", nargs="?", help="Input file with mypy error output (optional)"
    )
    parser.add_argument(
        "--mypy-args", help="Arguments to pass to mypy, e.g. 'shared/utils/app.py'"
    )
    ...
    if not summary_only:
        ...
        for error_type in ERROR_PRIORITIES:
            if error_type in categorized_errors and categorized_errors[error_type]:
                ...
                lines.extend(
                    categorized_errors[error_type][:20]
                )  # Show first 20 of each type
                if len(categorized_errors[error_type]) > 20:
                    lines.append(
                        f"... and {len(categorized_errors[error_type]) - 20} more errors of this type"
                    )
...
def main() -> int:
    ...
    try:
        ...
        # Output results
        output_errors(
            categorized_errors, file_counts, args.summary, args.output or default_output
        )
