...
ASTRO_LONGITUDE_FOR_HOLE_0 = 90.0 # Degrees (e.g., Cancer ingress if Aries is 0 deg East)

def longitude_to_aubrey_hole(longitude_deg: float, 
                             num_holes: int = DEFAULT_NUM_AUBREY_HOLES, 
                             hole_0_astro_ref_deg: float = ASTRO_LONGITUDE_FOR_HOLE_0) -> int:
    """
...
if __name__ == '__main__':
    print(f"Degrees per hole for {DEFAULT_NUM_AUBREY_HOLES} holes: {DEGREES_PER_HOLE:.3f}")
    print(f"Assuming Hole 0 is at {ASTRO_LONGITUDE_FOR_HOLE_0} degrees (e.g., North).")

    # Test cases
    test_longitudes = {
        ASTRO_LONGITUDE_FOR_HOLE_0: 0,  # Should be Hole 0
        ASTRO_LONGITUDE_FOR_HOLE_0 + DEGREES_PER_HOLE / 2 - 0.1: 0, # Just before midpoint to Hole 1, should be Hole 0
        ASTRO_LONGITUDE_FOR_HOLE_0 + DEGREES_PER_HOLE / 2 + 0.1: 1, # Just after midpoint to Hole 1, should be Hole 1
        ...
        (ASTRO_LONGITUDE_FOR_HOLE_0 + 27 * DEGREES_PER_HOLE): 27, # Should be Hole 27 (approx South)
        (ASTRO_LONGITUDE_FOR_HOLE_0 + 55 * DEGREES_PER_HOLE): 55, # Should be Hole 55
        (ASTRO_LONGITUDE_FOR_HOLE_0 - DEGREES_PER_HOLE / 2 + 0.1): 55, # Just before midpoint to Hole 0 (from CCW), should be Hole 55
        (ASTRO_LONGITUDE_FOR_HOLE_0 - DEGREES_PER_HOLE / 2 - 0.1): 55, # Further into Hole 55 range
    ...
    for lon, expected_hole in test_longitudes.items():
        ...
        if expected_hole is None:
            ...
            calc_expected_int = int(round(calc_expected_float)) % DEFAULT_NUM_AUBREY_HOLES
            expected_hole_val = calc_expected_int
        else:
            expected_hole_val = expected_hole
            
        calculated_hole = longitude_to_aubrey_hole(lon)
        status = "PASS" if calculated_hole == expected_hole_val else f"FAIL (Expected {expected_hole_val})"
    ...
    lon_just_before_hole0_ccw = (ASTRO_LONGITUDE_FOR_HOLE_0 - DEGREES_PER_HOLE / 4 + 360.0) % 360.0
    ...
    print(f"  Longitude {lon_just_before_hole0_ccw:6.1f} deg (just before Hole 0 CCW) -> Hole {calculated_hole:2} ({status})") 