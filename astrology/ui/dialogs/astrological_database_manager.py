...
from PyQt6.QtCore import QDate, QObject, QTimer, pyqtSignal
...
class CalculatorThread(QObject, Thread):
    ...
    def __init__(
        self, start_year: int, end_year: int, repository: AstrologicalEventsRepository
    ):
...
class AstrologicalDatabaseManager(QDialog):
    ...
    def _init_calculation_tab(self):
        ...
        self.test_aspects_btn.setToolTip(
            "Test whether aspects can be calculated with the current Swiss Ephemeris setup"
        )

        # Save aspects button
        self.save_aspects_btn = QPushButton("Save Current Aspects to Database")
        self.save_aspects_btn.setToolTip(
            "Calculate current planetary aspects and save them to the database"
        )

        # View aspects button
        self.view_aspects_btn = QPushButton("View Aspects in Database")
        self.view_aspects_btn.setToolTip(
            "View and filter planetary aspects stored in the database"
        )
    ...
    def _init_database_management_tab(self):
        ...
        self.find_duplicates_btn.setToolTip(
            "Search for and remove duplicate entries in the database"
        )
        ...
        try:
            ...
            with self.database.connection() as conn:
                ...
                try:
                    ...
                    self.lunar_phases_count_label.setText(
                        f"Lunar Phases: {lunar_phases_count}"
                    )
                except sqlite3.Error:
                ...
                try:
                    ...
                    self.planet_phases_count_label.setText(
                        f"Planet Phases: {planet_phases_count}"
                    )
                except sqlite3.Error:
                    self.planet_phases_count_label.setText(
                        "Planet Phases: Not available"
                    )
                ...
                try:
                    ...
                    self.solar_events_count_label.setText(
                        f"Solar Events: {solar_events_count}"
                    )
                except sqlite3.Error:
            ...
            try:
                ...
                QMessageBox.critical(
                    self, "Statistics Error", f"An error occurred: {str(e)}"
                )
    ...
    def _handle_find_duplicates(self):
        ...
        try:
            ...
            with self.database.connection() as conn:
                ...
                msg.setText(
                    f"Found {total_duplicate_sets} sets of duplicate entries across {len(all_duplicates)} tables."
                )
    ...
    def _handle_vacuum_database(self):
        ...
        try:
            ...
            with self.database.connection() as conn:
                ...
                try:
                    self.notification_service.show_success(
                        "Database Optimized", "Successfully optimized the database."
                    )
                except Exception as notification_error:
                    logger.error(
                        f"Failed to show success notification: {notification_error}"
                    )
                    QMessageBox.information(
                        self, "Success", "Database successfully optimized"
                    )
            ...
            try:
                ...
                with self.database.connection() as conn:
                    ...
                    try:
                        ...
                        QMessageBox.information(
                            self, "Success", "Database successfully cleared"
                        )
                ...
                try:
                    ...
                    logger.error(
                        f"Failed to show error notification: {notification_error}"
                    )
                    QMessageBox.critical(
                        self, "Database Error", f"An error occurred: {str(e)}"
                    )
    ...
    def _handle_calculate_range(self):
        ...
        if end_year <= start_year:
            QMessageBox.warning(
                self, "Invalid Range", "End year must be greater than start year"
            )
            return
        ...
        self._start_calculation(
            f"Calculating range {start_year}-{end_year}", start_year, end_year
        )
    ...
    def _start_calculation(self, message: str, start_year: int, end_year: int):
        ...
        # Notify user
        self.notification_service.show_info(
            f"Started calculating astrological events for {start_year}-{end_year}"
        )
    ...
    def _calculation_finished(self, success: bool):
        ...
        if success:
            ...
            self.notification_service.show_success(
                "Astrological events calculation completed"
            )
            ...
            self.notification_service.show_error(
                "Astrological events calculation failed"
            )
    ...
    def _verify_aspects_calculation(self):
        ...
        try:
            ...
            if stats["calculated_ranges"]:
                last_calc = max(
                    stats["calculated_ranges"], key=lambda r: r["calculation_date"]
                )
    ...
    def _update_status(self):
        ...
        try:
            ...
            if stats["calculated_ranges"]:
                ...
                self.last_calculation_label.setText(
                    f"Last Calculation: {dt.strftime('%Y-%m-%d %H:%M:%S')}"
                )
            else:
            ...
            if "event_counts" in stats:
                self.aspects_count_label.setText(
                    f"Aspects: {stats['event_counts'].get('aspects', 0):,}"
                )
                self.lunar_phases_count_label.setText(
                    f"Lunar Phases: {stats['event_counts'].get('lunar_phases', 0):,}"
                )
                self.planet_phases_count_label.setText(
                    f"Planet Phases: {stats['event_counts'].get('planet_phases', 0):,}"
                )
                self.eclipses_count_label.setText(
                    f"Eclipses: {stats['event_counts'].get('eclipses', 0):,}"
                )
                self.solar_events_count_label.setText(
                    f"Solar Events: {stats['event_counts'].get('solar_events', 0):,}"
                )
    ...
    def _handle_test_aspects(self):
        ...
        try:
            ...
            for planet_id, planet_name in planets.items():
                ...
                position = (
                    result[0] if isinstance(result[0], (int, float)) else result[0][0]
                )
            ...
            if aspects:
                ...
                QMessageBox.information(
                    self, "Aspect Test Results", "No significant aspects found."
                )
    ...
    def _check_all_aspects(self, planet_positions):
        ...
        try:
            ...
            major_aspects = {
                "Conjunction": (
                    AspectType.CONJUNCTION.angle,
                    AspectType.CONJUNCTION.base_orb,
                ),
                "Opposition": (
                    AspectType.OPPOSITION.angle,
                    AspectType.OPPOSITION.base_orb,
                ),
                ...
                for j in range(i + 1, len(planets)):
                    ...
                    for aspect_name, (aspect_angle, orb) in major_aspects.items():
                        if abs(diff - aspect_angle) <= orb:
                            ...
                            aspects.append(
                                f"{planet1} {aspect_name} {planet2} - {exactness_percentage}% exact ({round(exactness, 2)}° orb)"
                            )

            # Sort aspects by exactness (assuming higher percentage is first in the list)
            aspects.sort(
                reverse=True, key=lambda x: int(x.split(" - ")[1].split("%")[0])
            )
        ...
        try:
            ...
            if aspects:
                ...
                self.status_label.setText(
                    f"Saved {len(aspects)} aspects for {date.strftime('%Y-%m-%d %H:%M')}"
                )
                QMessageBox.information(
                    self, "Success", f"Saved {len(aspects)} aspects to database."
                )
            else:
                self.status_label.setText("No aspects found to save.")
                QMessageBox.information(
                    self, "No Aspects", "No significant aspects found to save."
                )
    ...
    def _save_aspects_to_database(self, date, aspects):
        ...
        try:
            ...
            for aspect_text in aspects:
                ...
                self._add_aspect_to_database(
                    cursor, date, planet1, planet2, aspect_type, exactness, orb
                )
    ...
    def _add_aspect_to_database(
        self, cursor, date, planet1, planet2, aspect_type, exactness, orb
    ):
        """
        ...
        try:
            ...
            if not self._validate_aspect(
                date, planet1, planet2, aspect_type, exactness, orb
            ):
                logger.warning(
                    f"Invalid aspect data skipped: {planet1} {aspect_type} {planet2}"
                )
                return None
            ...
            for planet_name in [planet1, planet2]:
                cursor.execute(
                    "SELECT id FROM celestial_bodies WHERE name = ?", (planet_name,)
                )
    ...
    def _validate_aspect(self, date, planet1, planet2, aspect_type, exactness, orb):
        ...
        try:
            ...
            if (
                not isinstance(exactness, (int, float))
                or exactness < 0
                or exactness > 100
            ):
    ...
    def _handle_view_aspects(self):
        ...
        try:
            ...
            table.setHorizontalHeaderLabels(
                ["Date", "Planet 1", "Aspect", "Planet 2", "Exactness", "Orb"]
            )
            ...
            view_button.clicked.connect(
                lambda: self._load_aspects_to_table(
                    table,
                    start_date_edit.date().toString("yyyy-MM-dd"),
                    end_date_edit.date().toString("yyyy-MM-dd"),
                    planet_combo.currentText()
                    if planet_combo.currentIndex() > 0
                    else None,
                    aspect_combo.currentText()
                    if aspect_combo.currentIndex() > 0
                    else None,
                )
            )
    ...
    def _load_aspects_to_table(
        self, table, start_date, end_date, planet=None, aspect_type=None
    ):
        """
        ...
        try:
            ...
            if not os.path.exists(db_path):
                QMessageBox.warning(
                    self,
                    "No Data",
                    "Aspects database not found or no aspects have been saved.",
                )
                return
            ...
            # Populate table
            for row_idx, (date, planet1, aspect, planet2, exactness, orb) in enumerate(
                results
            ):
            ...
            if results:
                self.status_label.setText(
                    f"Found {len(results)} aspects matching criteria"
                )
            else:
