...
from astrology.services.moon_phase_calculator import Aspect
...
from astrology.services.astrological_event_calculator import AstrologicalEventCalculator, AspectType
class PlannerService:
    ...
    def _load_settings(self) -> PlannerSettings:
        ...
        try:
            ...
            if row:
                ...
                if row.get('default_location_latitude') and row.get('default_location_longitude'):
                    from astrology.services.location_service import Location
                    ...
                    logger.debug(f"Loaded default location: {default_location.display_name}")
                else:
        ...
        try:
            with self.repository.database.transaction() as conn:
                ...
                if settings.default_location:
                    ...
                    logger.debug(f"Saving default location: {settings.default_location.display_name}")
                else:
    ...
    def get_all_astrological_events_for_date(self, date: datetime.date, include_minor_aspects: bool = False) -> List[PlannerEvent]:
        ...
        try:
            ...
            if self._settings.show_lunar_phases:
                moon_phase_events = self.moon_phase_calculator.get_moon_phase_events_for_date(date)
                events.extend(moon_phase_events)
                logger.debug(f"Added {len(moon_phase_events)} moon phase events for {date}")
        except Exception as e:
        ...
        try:
            ...
            if self._settings.show_planetary_aspects:
                logger.debug(f"Fetching planetary aspects for {date} with include_minor={include_minor_aspects}")
                aspect_events = self.get_planetary_aspects_for_date(date, include_minor=include_minor_aspects)
                ...
                for i, event in enumerate(aspect_events[:3]):
                    logger.debug(f"  Aspect event {i+1}: {event.title}, type={event.event_type}, time={event.start_time}")
                
                events.extend(aspect_events)
                logger.debug(f"Added {len(aspect_events)} planetary aspect events for {date}, total events now: {len(events)}")
        except Exception as e:
        ...
        try:
            ...
            if self._settings.show_retrogrades:
                ...
                logger.debug(f"Added {len(retrograde_events)} retrograde events for {date}")
        except Exception as e:
        ...
        try:
            ...
            if self._settings.show_venus_cycles:
                ...
                planet_phases = self.astrological_event_calculator.get_planet_phases_for_date(date_as_datetime)
                ...
                for phase in planet_phases:
                    event = PlannerEvent(
                        ...
                        end_time=phase.get('timestamp', date_as_datetime) + timedelta(hours=1),  # Set a 1-hour duration for display
                        color="#9370DB"  # Medium purple color for planetary phases
                    )
                    events.append(event)
                logger.debug(f"Added {len(planet_phases)} planetary phase events for {date}")
        except Exception as e:
    ...
    def _convert_aspects_to_events(self, aspects: List) -> List[PlannerEvent]:
        ...
        for aspect in aspects:
            try:
                ...
                if hasattr(aspect, 'to_planner_event') and callable(getattr(aspect, 'to_planner_event')):
                ...
                color = aspect_colors.get(aspect_key, "#3498db")  # Default blue if aspect not found
                ...
                event = PlannerEvent(
                    ...
                    end_time=exact_time + timedelta(hours=1),  # 1-hour duration for visibility
                    color=color
                )
                events.append(event)
    ...
    def get_events_for_month(
        ...
        while current_day.month == month:
            ...
            if self._settings.show_lunar_phases:
                moon_phases = self.moon_phase_calculator.get_moon_phase_events_for_date(current_day)
                if moon_phases:
            ...
            if self._settings.show_planetary_aspects:
                ...
                aspects = self.get_planetary_aspects_for_date(current_day, include_minor=include_minor_aspects)
                if aspects:
                    logger.debug(f"Adding {len(aspects)} aspects to events for {current_day}")
            ...
            if self._settings.show_venus_cycles:
                ...
                # Use the AstrologicalEventCalculator instance to get planet phases
                planet_phases = self.astrological_event_calculator.get_planet_phases_for_date(day_datetime)
                ...
                for phase in planet_phases:
                    ...
                    if "Venus" in phase.get('title', ''):
                        event = PlannerEvent(
                            ...
                            end_time=phase.get('timestamp', day_datetime) + timedelta(hours=1),
    ...
    def get_planetary_aspects(self, date: datetime.date, include_minor: bool = True) -> List[PlannerEvent]:
        ...
        """
        logger.debug(f"Calculating planetary aspects for {date} using Swiss Ephemeris (include_minor={include_minor})")
    ...
    def get_retrograde_periods(self, date: datetime.date) -> List[PlannerEvent]:
        ...
        try:
                ...
                if ret == 0:  # Successful calculation
                    ...
                    if speed < 0:
                        ...
                        event_type=EventType.RETROGRADE,
                            color=planet_colors.get(planet_name, "#7F8C8D"),  # Default color if not found
                            metadata={
                                "planet": planet_name,
                                "position": longitude,
                                "speed": speed
            ...
            if self.astrology_calculation_service is None:
                logger.warning("Using placeholder Mercury retrograde data as AstrologyCalculationService is unavailable")
    ...
    def _get_zodiac_sign(self, date: datetime) -> str:
        ...
        return "Aries"
    def get_planetary_aspects_for_date(self, date: datetime.date, include_minor: bool = False) -> List[PlannerEvent]:
            ...
            if not include_minor and aspects:
                try:
                    ...
                    if hasattr(aspects[0], 'aspect_type'):
                        ...
                        aspects = [a for a in aspects if getattr(a, 'aspect_type', None) in [
                        ...
                        logger.warning("Could not filter minor aspects: unexpected aspect structure")
        ...
        try:
            ...
            if not isinstance(mercury_result, tuple) or len(mercury_result) < 2:
                ...
                if isinstance(sun_result, tuple) and len(sun_result) > 0:
                    ...
                    if angular_diff < 0.5:  # Within half a degree of inferior conjunction
                        result.append({
                    ...
                    elif abs(angular_diff - 180) < 0.5:  # Within half a degree of superior conjunction
                        result.append({
