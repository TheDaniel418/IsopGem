...
REQUIRED_CSV_COLUMNS = [
    'year', 'month', 'day', 'eclipse_type', 'cat_no',
    'magnitude', 'lat_dd_ge', 'lng_dd_ge', 'central_duration', 'path_width', 'td_ge'
]
...
class EclipseCatalogService:
    ...
    """
    def __init__(self, csv_file_path: str = "assets/ephemeris/eclipse_besselian_from_mysqldump2.csv"):
        """
    ...
    def _load_data(self) -> None:
        ...
        try:
            ...
            self._eclipse_df = pd.read_csv(
                self.csv_file_path,
                usecols=lambda column_name: column_name in REQUIRED_CSV_COLUMNS, # More robust usecols
            ...
            string_columns_to_clean = ['eclipse_type', 'cat_no', 'central_duration', 'path_width']
            for col in string_columns_to_clean:
                if col in self._eclipse_df.columns:
                    ...
                    self._eclipse_df[col] = self._eclipse_df[col].astype(str).str.strip()
            print(f"Successfully loaded and initially processed {len(self._eclipse_df)} records.")
        except FileNotFoundError:
            print(f"🚨 Error: Eclipse catalog CSV file not found at '{self.csv_file_path}'.")
            self._eclipse_df = pd.DataFrame(columns=REQUIRED_CSV_COLUMNS) # Ensure empty df has columns
        except ValueError as ve:
            # This can happen if a required column for usecols is missing in the CSV
            print(f"🚨 ValueError during CSV loading (e.g., a required column might be missing): {ve}")
            print(f"Please ensure the CSV file at '{self.csv_file_path}' contains all columns: {REQUIRED_CSV_COLUMNS}")
    ...
    def get_solar_eclipses(self, start_year: Optional[int] = None, end_year: Optional[int] = None) -> List[EclipseData]:
        """
        ...
        if 'eclipse_type' not in self._eclipse_df.columns or 'year' not in self._eclipse_df.columns:
        ...
        try:
            ...
            df_filtered.dropna(subset=['year'], inplace=True) # Remove rows where year could not be coerced
            df_filtered['year'] = df_filtered['year'].astype(int)
        except Exception as e:
            print(f"Error converting 'year' column to numeric: {e}. Year filtering may be affected.")
        ...
        for _, row in df_filtered.iterrows():
            try:
                event_data = {
                    ...
                    'central_duration': self._to_str_or_none(row.get('central_duration')),
                ...
                print(f"⚠️ Skipping row due to data conversion or validation error: {row.to_dict() if hasattr(row, 'to_dict') else 'N/A'}. Error: {e}")
...
if __name__ == '__main__':
    ...
    try:
        ...
        print("❌ Critical Error: Pandas library is not installed. Please install it (e.g., pip install pandas).")
        exit(1)
    ...
    if catalog_service._eclipse_df is not None and not catalog_service._eclipse_df.empty:
        ...
        if all_solar_eclipses:
            ...
            for i, eclipse in enumerate(all_solar_eclipses[:5]):
                print(f"  {i+1}. {eclipse.year}-{eclipse.month:02d}-{eclipse.day:02d}: Type={eclipse.eclipse_type}, Cat={eclipse.cat_no}, Mag={eclipse.magnitude:.4f if eclipse.magnitude else 'N/A'}")
        else:
            print("No solar eclipses found or data loading issue persisted.")

        print("\n📅 Fetching solar eclipses for the year 1605...")
        eclipses_1605 = catalog_service.get_solar_eclipses(start_year=1605, end_year=1605)
        if eclipses_1605:
            print(f"Found {len(eclipses_1605)} solar eclipse(s) in 1605:")
            for eclipse in eclipses_1605:
                print(f"  - {eclipse.year}-{eclipse.month:02d}-{eclipse.day:02d}: Type={eclipse.eclipse_type}, Cat={eclipse.cat_no}, Mag={eclipse.magnitude:.4f if eclipse.magnitude else 'N/A'}, Lat={eclipse.lat_dd_ge}, Dur={eclipse.central_duration}")
        else:
            print("No solar eclipses found for 1605.")

        print("\n🗓️ Fetching solar eclipses from 2000 to 2001 (showing first 5 if available)...")
        eclipses_2000_2001 = catalog_service.get_solar_eclipses(start_year=2000, end_year=2001)
        if eclipses_2000_2001:
            ...
            for i, eclipse in enumerate(eclipses_2000_2001[:5]):
                print(f"  {i+1}. {eclipse.year}-{eclipse.month:02d}-{eclipse.day:02d}: Type={eclipse.eclipse_type}, Cat={eclipse.cat_no}")
        else:
        ...
        if not eclipses_year_1:
            print("Correctly found no solar eclipses for year 1 (as expected from typical catalog ranges).")
        else:
            print(f"Found {len(eclipses_year_1)} solar eclipses for year 1, which might be unexpected.")

    else:
        print("❌ Eclipse data DataFrame is None or empty after initialization. Service cannot function.")
    print("\n--- Test Complete --- ") 