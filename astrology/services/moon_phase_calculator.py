...
from typing import List, Optional, Dict, NamedTuple, Tuple, Union, Any
...
class Aspect:
        ...
        try:
            ...
            if isinstance(aspect_type, AspectType):
                ...
                for at in AspectType:
                    if at.value == aspect_type or at.name.lower() == aspect_type.lower():
                        self.aspect_type = at
                        aspect_found = True
                        break
                ...
                if not aspect_found:
                    ...
                    for at in AspectType:
                        if at.value.lower() in aspect_type.lower() or at.name.lower() in aspect_type.lower():
                            self.aspect_type = at
                            aspect_found = True
                            logger.debug(f"Matched aspect type {at} from string '{aspect_type}'")
                            break
                
                # If absolutely nothing matches, default to conjunction
                if not aspect_found:
                    logger.warning(f"Could not determine aspect type from '{aspect_type}', defaulting to conjunction")
                    self.aspect_type = AspectType.CONJUNCTION
            else:
                # Default to conjunction if we can't interpret the aspect type
                logger.warning(f"Unrecognized aspect type: {aspect_type}, defaulting to conjunction")
                self.aspect_type = AspectType.CONJUNCTION
    ...
    def get_description(self) -> str:
        ...
        try:
            display_name = ASPECT_DISPLAY_NAMES.get(self.aspect_type, self.aspect_type.value.capitalize())
    ...
    def to_planner_event(self) -> PlannerEvent:
        ...
        try:
            ...
            color = aspect_colors.get(self.aspect_type, "#3498db")  # Default blue if aspect not found
            ...
            return PlannerEvent(
                ...
                metadata={
                    ...
                    'aspect_type': self.aspect_type.value if hasattr(self.aspect_type, 'value') else str(self.aspect_type),
...
class MoonPhaseCalculator:
    ...
    def _calculate_aspect_orb(self, pos1, pos2) -> Union[Tuple[AspectType, float], None]:
        ...
        try:
            ...
            logger.error(f"Error calculating aspect orb: {e}, pos1={type(pos1)}, pos2={type(pos2)}")
            return None

    def calculate_aspects(self, date: datetime.date, include_planets: List[int] = None) -> List[Aspect]:
        ...
        try:
            ...
            all_planet_ids = [swe.SUN, swe.MOON, swe.MERCURY, swe.VENUS, swe.MARS, 
                ...
                for planet2 in planet_ids[i+1:]:
                    ...
                    if aspect_result:
                        ...
                        if planet1_info and planet2_info:
                            ...
                            weight_factor = (planet1_info.weight + planet2_info.weight) / 2
