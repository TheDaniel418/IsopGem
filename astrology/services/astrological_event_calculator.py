...
class AstrologicalEventCalculator(SingletonService):
    ...
    def __init__(self, repository=None, ephemeris_path=None):
        ...
        try:
            ...
            logger.info(
                f"Swiss Ephemeris initialized successfully. Test Sun position: {result[0]}"
            )
        ...
        try:
            ...
            for year in range(start_year, end_year + 1):
                ...
                try:
                    ...
                    logger.error(
                        f"Error calculating aspects for {year}: {e}", exc_info=True
                    )
            ...
            logger.info(
                f"Calculated {event_count} events for years {start_year}-{end_year}"
            )
            return True

        except Exception as e:
            logger.error(
                f"Error calculating range {start_year}-{end_year}: {e}", exc_info=True
            )
    ...
    def _calculate_aspects_for_year(self, year: int) -> int:
        ...
        try:
            ...
            checked_daily_aspects = (
                set()
            )  # set of (year, month, day, body1, body2, aspect_type)
            ...
            while current_date <= end_date:
                ...
                for planet_name, planet_id in self.PLANETS.items():
                    try:
                        ...
                        logger.error(
                            f"SwissEph Error calculating position for {planet_name}: {e}"
                        )
                    except TypeError as e:
                        logger.error(
                            f"Type Error calculating position for {planet_name}: {e}"
                        )
                    except Exception as e:
                        logger.error(
                            f"Error calculating position for {planet_name}: {e}",
                            exc_info=True,
                        )
                ...
                for planet1, planet2 in self.ASPECT_PAIRS:
                    ...
                    if (
                        planet1 not in planet_positions
                        or planet2 not in planet_positions
                    ):
                        continue
                    ...
                    separation = self._normalize_angle(
                        abs(self._normalize_angle(pos1) - self._normalize_angle(pos2))
                    )
                    ...
                    for aspect_type, aspect_angle in ASPECT_ANGLES.items():
                        ...
                        # Calculate orb
                        orb = self._calculate_aspect_orb(
                            self._normalize_angle(pos1),
                            self._normalize_angle(pos2),
                            aspect_type,
                        )
                        ...
                        if orb is not None:
                            ...
                            if strength >= min_strength:
                                ...
                                if aspect_key not in processed_aspects:
                                    ...
                                    logger.debug(
                                        f"New aspect found: {p1} {aspect_type.name} {p2} ({orb:.2f}°)"
                                    )
                                else:
            ...
            try:
                logger.debug(
                    f"Starting database transaction for storing aspects for {year}"
                )
                aspect_count = 0
                ...
                try:
                    rows = self.repository.database.query_all(
                        "SELECT id, name FROM celestial_bodies"
                    )
                    for row in rows:
                    ...
                    for planet_name in self.PLANETS.keys():
                        if planet_name not in planet_db_ids:
                            logger.error(
                                f"No database ID found for planet {planet_name}"
                            )
                except Exception as e:
                    logger.error(
                        f"Error retrieving celestial body IDs: {e}", exc_info=True
                    )

                if not planet_db_ids:
                    logger.error(
                        "No celestial body IDs found in database. Cannot store aspects."
                    )
                    return 0
                ...
                for aspect_key, aspect_data in processed_aspects.items():
                    ...
                    if planet1 not in planet_db_ids or planet2 not in planet_db_ids:
                        logger.warning(
                            f"Skipping aspect between {planet1} and {planet2} - missing DB IDs"
                        )
                        continue
                ...
                logger.info(
                    f"Successfully stored {aspect_count} aspects for {year} in database"
                )
                return aspect_count
            ...
            while current_jd < end_jd:
                for target_angle, phase_type in phase_types.items():
                    try:
                        ...
                        phase_jd = self._find_exact_lunar_phase(
                            current_jd, target_angle
                        )
                        ...
                        logger.warning(
                            f"Error finding {phase_type.name} at JD {current_jd}: {e}"
                        )
    ...
    def _find_exact_lunar_phase(
        self, start_jd: float, target_angle: float
    ) -> Optional[float]:
        ...
        try:
            ...
            logger.warning(
                f"Error finding exact lunar phase (target angle {target_angle}): {e}"
            )
            return None

    def _find_quarter_moon(
        self, start_jd: float, target_angle: float
    ) -> Optional[float]:
        ...
        try:
            ...
            if isinstance(result, tuple):
                if len(result) >= 2:
                    ...
                    logger.warning(
                        f"Unexpected Swiss Ephemeris result format: {result}"
                    )
            ...
            logger.warning(
                f"Using FALLBACK method for Moon position. Result: {fallback_position:.2f}°"
            )
    ...
    def _get_sun_position(self, jd: float) -> float:
        ...
        try:
            ...
            logger.warning(
                f"Using FALLBACK method for Sun position. Result: {fallback_position:.2f}°"
            )
            return fallback_position
    ...
    def _store_lunar_phase(
        self,
        timestamp: datetime,
        phase_type: LunarPhaseType,
        moon_position: Optional[float] = None,
        sun_position: Optional[float] = None,
    ) -> None:
        ...
        try:
            ...
            if moon_position is None or sun_position is None:
                try:
                    ...
                    logger.warning(
                        f"Swiss Ephemeris calculation failed for lunar phase {phase_type} "
            ...
            if moon_position is None or sun_position is None:
                logger.error(
                    f"Could not determine positions for lunar phase {phase_type} at {timestamp}"
                )
                return
    ...
    def _calculate_planet_phases_for_year(self, year: int) -> int:
        ...
        planets = {"Mercury": swe.MERCURY, "Venus": swe.VENUS}
            ...
            while current_jd <= end_jd:
                try:
                    ...
                    if not isinstance(planet_result, tuple):
                        logger.error(
                            f"Invalid planet calculation result for {planet_name}: {planet_result}"
                        )
                        current_jd += day_step
                        continue
                    ...
                    if len(planet_result) >= 2 and isinstance(
                        planet_result[0], (int, float)
                    ):
                        # Direct tuple of coordinates
                        current_position = planet_result[0]  # longitude
                        current_speed = (
                            planet_result[3] if len(planet_result) > 3 else 0.0
                        )  # speed
                    elif len(planet_result) >= 1 and isinstance(
                        planet_result[0], (list, tuple)
                    ):
                        ...
                        logger.error(
                            f"Unexpected format for {planet_name} result: {planet_result}"
                        )
                        current_jd += day_step
                        continue
                    ...
                    elif len(sun_result) >= 1 and isinstance(
                        sun_result[0], (list, tuple)
                    ):
                        # Tuple where first element is coordinates
                        coords = sun_result[0]
                    ...
                    if (
                        last_event_day is not None
                        and (current_jd - last_event_day) < 10
                    ):
                    ...
                    if prev_speed is not None:
                        if prev_speed >= 0 and current_speed < 0:
                            ...
                            logger.info(
                                f"{planet_name} station retrograde at {current_date.isoformat()}"
                            )
                            ...
                            logger.info(
                                f"{planet_name} station direct at {current_date.isoformat()}"
                            )
                            self._store_planet_phase(
                                current_date,
                                planet_id,
                    ...
                    if prev_position is not None:
                        ...
                        if (angular_diff < 1 or angular_diff > 359) and (
                            prev_angular_diff > 1 and prev_angular_diff < 359
                        ):
                            logger.info(
                                f"{planet_name} superior conjunction at {current_date.isoformat()}"
                            )
                            self._store_planet_phase(
                                current_date,
                                planet_id,
                        ...
                        # Inferior conjunction (planet between Earth and Sun)
                            ...
                            abs(angular_diff - 180) < 1
                            and abs(prev_angular_diff - 180) > 1
                        ):
                            logger.info(
                                f"{planet_name} inferior conjunction at {current_date.isoformat()}"
                            )
                            self._store_planet_phase(
                                current_date,
                                planet_id,
                    ...
                    if prev_elongation is not None and elongation != prev_elongation:
                        ...
                        if elongation < prev_elongation and prev_speed > 0:
                            logger.info(
                                f"{planet_name} greatest eastern elongation {prev_elongation:.2f}° at {current_date.isoformat()}"
                            )
                            self._store_planet_phase(
                                current_date,
                                planet_id,
                        ...
                        # Decreasing to increasing = Western (morning) maximum elongation
                            ...
                            logger.info(
                                f"{planet_name} greatest western elongation {prev_elongation:.2f}° at {current_date.isoformat()}"
                            )
                            self._store_planet_phase(
                                current_date,
                                planet_id,
                    ...
                    logger.error(
                        f"Error calculating phase for {planet_name} at JD {current_jd}: {e}"
                    )
    ...
    def _is_inferior_conjunction(self, jd: float, planet_id: int) -> bool:
        ...
        try:
            ...
            return (
                lon_diff > 90
            )  # More than 90° difference indicates inferior conjunction
    ...
    def _find_exact_station(
        self, start_jd: float, end_jd: float, planet_id: int
    ) -> float:
    ...
    def _find_exact_max_elongation(
        self, start_jd: float, end_jd: float, planet_id: int, sun_id: int
    ) -> float:
        ...
        while (
            abs(b - a) > 0.01 and iter_count < max_iter
    ...
    def _calculate_elongation_at_jd(
        self, jd: float, planet_id: int, sun_id: int
    ) -> float:
        ...
        try:
            ...
            planet_pos = (
                planet_result[0]
                if isinstance(planet_result, tuple) and len(planet_result) > 0
                else 0
            )
            sun_pos = (
                sun_result[0]
    ...
    def _store_planet_phase(
        self,
        timestamp: datetime,
        body_id: int,
        phase_type: PlanetPhaseType,
        elongation_degree: Optional[float] = None,
        planet_position: Optional[float] = None,
    ) -> None:
        ...
        try:
            ...
            body_db_id = None
            rows = self.repository.database.query_all(
                "SELECT id FROM celestial_bodies WHERE name = ?", (body_name,)
            )
            ...
            if rows:
                ...
                logger.warning(
                    f"Celestial body {body_name} not found in database, skipping"
                )
                return
            ...
            logger.debug(
                f"Stored {phase_type.value} for {body_name} at {timestamp.isoformat()}"
            )

        except Exception as e:
            logger.error(
                f"Error storing planet phase {phase_type} for {body_id} at {timestamp}: {e}"
            )
    ...
    def _calculate_eclipses_for_year(self, year: int) -> int:
        ...
        try:
            ...
            start_date = datetime(
                year - 1, 12, 15
            )  # Start slightly before the year to catch eclipses at the very beginning
            end_date = datetime(
                year + 1, 1, 15
            )  # Continue slightly into next year to catch eclipses at the very end
            ...
            try:
                ...
                logger.error(
                    f"Error calculating solar eclipses for {year}: {e}", exc_info=True
                )
            ...
            try:
                ...
                logger.error(
                    f"Error calculating lunar eclipses for {year}: {e}", exc_info=True
                )
            ...
            logger.error(
                f"Error in eclipse calculations for {year}: {e}", exc_info=True
            )
            return 0

    def _calculate_solar_eclipses(
        self, start_jd: float, end_jd: float, year: int
    ) -> int:
        ...
        while current_jd < end_jd:
            try:
                ...
                if isinstance(result, tuple) and len(result) >= 2:
                    ...
                    logger.error(
                        f"Unexpected result format from sol_eclipse_when_glob: {result}"
                    )
                ...
                if eclipse_dt.year == year:
                    ...
                    eclipse_count += 1
                    logger.debug(
                        f"Stored solar eclipse: {eclipse_type.value} on {eclipse_dt.isoformat()}"
                    )
                ...
                logger.error(
                    f"Error calculating solar eclipse at JD {current_jd}: {e}",
                    exc_info=True,
                )
        ...
        while current_jd < end_jd:
            try:
                ...
                if isinstance(result, tuple):
                    ...
                    logger.error(
                        f"Unexpected result format from lun_eclipse_when: {result}"
                    )
                ...
                if eclipse_dt.year == year:
                    ...
                    eclipse_count += 1
                    logger.debug(
                        f"Stored lunar eclipse: {eclipse_type.value} on {eclipse_dt.isoformat()}"
                    )
                ...
                logger.error(
                    f"Error calculating lunar eclipse at JD {current_jd}: {e}",
                    exc_info=True,
                )
                current_jd += 30  # Skip forward to avoid getting stuck
        ...
        try:
            ...
            try:
                ...
                if isinstance(sun_result, tuple) and len(sun_result) > 0:
                    sun_pos = (
                        sun_result[0][0]
                        if isinstance(sun_result[0], (list, tuple))
                        else sun_result[0]
                    )
                ...
                if isinstance(moon_result, tuple) and len(moon_result) > 0:
                    moon_pos = (
                        moon_result[0][0]
                        if isinstance(moon_result[0], (list, tuple))
                        else moon_result[0]
                    )
                ...
                if isinstance(node_result, tuple) and len(node_result) > 0:
                    node_pos = (
                        node_result[0][0]
                        if isinstance(node_result[0], (list, tuple))
                        else node_result[0]
                    )
            ...
            eclipse_classification = (
                eclipse_type.value.split("_", 1)[1]
                if "_" in eclipse_type.value
                else "unknown"
            )
            ...
            try:
                ...
                table_info = self.repository.database.query_all(
                    "PRAGMA table_info(eclipses)"
                )
                ...
                logger.debug(
                    f"Successfully stored {eclipse_type.value} eclipse at {timestamp.isoformat()}"
                )
            ...
            logger.error(
                f"Error storing eclipse {eclipse_type} at {timestamp}: {e}",
                exc_info=True,
            )
        ...
        try:
            if not hasattr(swe, "next_solpoint_ut"):
                raise AttributeError(
                    "Swiss Ephemeris module does not have next_solpoint_ut function"
                )
            ...
            logger.warning(
                f"Swiss Ephemeris error: {e}. Using approximate solar events for {year}."
            )
    ...
    def _calculate_solar_events_fallback(self, year: int) -> None:
        ...
        logger.info(
            f"Calculated approximate solar events for {year} using fallback method"
        )
    ...
    def _calculate_lunar_phases_fallback(self, year: int) -> int:
        ...
        """
        logger.info(
            f"Using fallback method for lunar phases calculation for year {year}"
        )
        ...
        last_new_moon = reference_new_moon + timedelta(
            days=complete_months * synodic_month
        )
            ...
            if current_date.year == year:
                try:
                    ...
                    if first_quarter.year == year:
                        self._store_lunar_phase(
                            first_quarter, LunarPhaseType.FIRST_QUARTER
                        )
                        phase_count += 1
                    ...
                    if last_quarter.year == year:
                        self._store_lunar_phase(
                            last_quarter, LunarPhaseType.LAST_QUARTER
                        )
                        phase_count += 1
                except Exception as e:
                    logger.error(
                        f"Error storing fallback lunar phase at {current_date}: {e}"
                    )
        ...
        logger.info(
            f"Fallback method calculated {phase_count} lunar phases for year {year}"
        )
        return phase_count
    ...
    def _approximate_sun_position(self, timestamp: datetime) -> float:
        ...
        year_fraction = (day_of_year - 1 + day_fraction) / (
            366 if calendar.isleap(timestamp.year) else 365
        )
    ...
    def _store_solar_event(
        self, event_dt: datetime, event_type: SolarEventType, sun_position: float
    ):
    ...
    def _calculate_aspect_orb(
        self, pos1: float, pos2: float, aspect_type: AspectType
    ) -> Optional[float]:
    ...
    def get_planet_phases_for_date(self, date: datetime) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            with self.repository.database.connection() as conn:
                ...
                logger.debug(
                    f"Found {len(phases)} planet phases for {date.strftime('%Y-%m-%d')}"
                )
                return phases

        except Exception as e:
            logger.error(
                f"Error retrieving planet phases for {date.strftime('%Y-%m-%d')}: {e}"
            )
            return []
    ...
    def populate_planet_phases(self, start_year: int, end_year: int) -> int:
        ...
        logger.info(
            f"Calculated {total_phases} total planet phases for {start_year}-{end_year}"
        )
        return total_phases
    ...
    def _setup_ephemeris(self) -> None:
        ...
        try:
            ...
            if ephe_dir.exists():
                ...
                logger.info(
                    "No ephemeris files found in ~/.isopgem/ephemeris, using built-in data"
                )
                swe.set_ephe_path()
            ...
            if isinstance(sun_result, tuple) and len(sun_result) >= 2:
                sun_lon = (
                    sun_result[0][0]
                    if isinstance(sun_result[0], (tuple, list))
                    else sun_result[0]
                )
                logger.info(
                    f"Swiss Ephemeris initialized successfully. Test Sun longitude: {sun_lon:.4f}°"
                )
            else:
                logger.warning(
                    f"Unexpected Sun calculation result format: {sun_result}"
                )
            ...
            if isinstance(moon_result, tuple) and len(moon_result) >= 2:
                moon_lon = (
                    moon_result[0][0]
                    if isinstance(moon_result[0], (tuple, list))
                    else moon_result[0]
                )
                logger.info(f"Test Moon longitude: {moon_lon:.4f}°")
            else:
                logger.warning(
                    f"Unexpected Moon calculation result format: {moon_result}"
                )
            ...
            logger.warning(
                "Some calculations will use mathematical approximations instead of precise ephemeris data"
            )
    ...
    def get_aspects_for_date(self, date: date) -> List:
        ...
        try:
            ...
            with self.repository.database.connection() as conn:
                ...
                cursor.execute(
                    query, (start_of_day.isoformat(), end_of_day.isoformat())
                )
                aspects = []

                for row in cursor.fetchall():
                    # Create PlanetInfo objects for the planets
                    from astrology.services.moon_phase_calculator import (
                        Aspect as MoonAspect,
                    )
                    from astrology.services.moon_phase_calculator import PlanetInfo
                    ...
                    planet1 = PlanetInfo(
                        id=row[1], name=body1_name, weight=1.0, position=row[8]
                    )
                    planet2 = PlanetInfo(
                        id=row[3], name=body2_name, weight=1.0, position=row[9]
                    )
                    ...
                    logger.debug(
                        f"Found aspect: {body1_name} {aspect_type_str} {body2_name}"
                    )
