...
from astrology.utils.astronomical_calculations import (
    ASTRO_LONGITUDE_FOR_HOLE_0,
    longitude_to_aubrey_hole,
)
...
class StonehengeSimulationService:
    ...
    def __init__(self, proximity_threshold: int = 0):
        ...
        self.sun_marker = Marker(
            name="S", initial_position=NUM_HOLES // 2
        )  # Opposite Moon
        self.node_n_marker = Marker(name="N", initial_position=0)  # Same as Moon
        self.node_n_prime_marker = Marker(
            name="N'", initial_position=NUM_HOLES // 2
        )  # Opposite N
        ...
        self.nodal_influence_width_one_sided: int = (
            3  # NEW: For wider nodal influence zones
        )
    ...
    def _advance_hoyle_day_for_temp_simulation(
        ...
        if year_cycle_completed:
            ...
            temp_node_n_prime_marker.current_position = (
                temp_node_n_marker.current_position + NUM_HOLES // 2
            ) % NUM_HOLES

            # Solar Auto-Recalibration (at start/mid of Hoyle Year)
            current_hoyle_day_in_year = (
                temp_time_state.day_within_year_cycle
            )  # Use temp_time_state

            if current_hoyle_day_in_year == 1:  # Start of Hoyle Year
                if (
                    temp_sun_marker.current_position
                    != self._summer_solstice_alignment_hole
                ):
                    temp_sun_marker.current_position = (
                        self._summer_solstice_alignment_hole
                    )
            elif (
                current_hoyle_day_in_year == (TimeState.DAYS_IN_HOYLE_YEAR // 2) + 1
            ):  # Mid Hoyle Year
                if (
                    temp_sun_marker.current_position
                    != self._winter_solstice_alignment_hole
                ):
                    temp_sun_marker.current_position = (
                        self._winter_solstice_alignment_hole
                    )
        ...
        is_full_moon_condition_for_correction = (
            diff <= alignment_check_threshold
            or diff >= (NUM_HOLES - alignment_check_threshold)
        )
    ...
    def _move_markers(
        self, sun_cycle_completed: bool, year_cycle_completed: bool
    ) -> list[str]:
        """
        ...
        if year_cycle_completed:
            ...
            self.node_n_prime_marker.current_position = (
                self.node_n_marker.current_position + NUM_HOLES // 2
            ) % NUM_HOLES
            ...
            if (
                current_hoyle_day_in_year == 1
            ):  # Start of Hoyle Year (Summer Solstice proxy)
                if (
                    self.sun_marker.current_position
                    != self._summer_solstice_alignment_hole
                ):
                    self.sun_marker.current_position = (
                        self._summer_solstice_alignment_hole
                    )
                    log_msg = "☀️ Sun marker auto-aligned to Summer Solstice point (Hole {}).".format(
                        self._summer_solstice_alignment_hole
                    )
                ...
                current_hoyle_day_in_year == (TimeState.DAYS_IN_HOYLE_YEAR // 2) + 1
            ):  # Mid Hoyle Year (Winter Solstice proxy)
                if (
                    self.sun_marker.current_position
                    != self._winter_solstice_alignment_hole
                ):
                    ...
                    log_msg = "☀️ Sun marker auto-aligned to Winter Solstice point (Hole {}).".format(
                        self._winter_solstice_alignment_hole
                    )
                    if self._last_correction_log["solar"] != log_msg:
                        recalibration_logs.append(log_msg)
        ...
        if is_full_moon_condition_for_correction:
            if moon_pos != expected_moon_at_full:  # If not already perfectly opposite
                ...
                log_msg = "🌕 Lunar marker auto-recalibrated for perfect Full Moon alignment (M at {}, S at {}).".format(
                    expected_moon_at_full, sun_pos
                )
    ...
    def _check_for_eclipses(self) -> list[str]:
        ...
        is_full_moon = is_syzygy_aligned(
            m_pos, sun_opposite_pos, self.proximity_threshold
        )

        if is_full_moon and (
            moon_in_n_zone or moon_in_n_prime_zone
        ):  # Re-use nodal zone checks
    ...
    def advance_simulation_step(self) -> dict:
        ...
        recalibration_log_messages = self._move_markers(
            sun_cycle_completed, year_cycle_completed
        )  # Now returns logs
    ...
    def reset_simulation(
        self, start_day: int = 1, start_year: int = 1, proximity_threshold: int = 0
    ) -> dict[str, int]:
        """
        ...
        if (
            self.time_state.current_year == 1
            and self.time_state.day_within_year_cycle == 1
        ):
            self.sun_marker.current_position = self._summer_solstice_alignment_hole
            print(
                f"Initial solar alignment on reset: Sun marker set to Hole {self._summer_solstice_alignment_hole} for Summer Solstice."
            )

        print(
            f"Simulation reset to: Time: Day {self.time_state.current_day}, Year {self.time_state.current_year}, ProxThreshold: {self.proximity_threshold}"
        )
        return self.get_current_marker_positions()

    def reset_simulation_from_ephemeris(
        self,
        year: int,
        month: int,
        day: int,
        ...
        try:
            ...
            if celestial_data:
                ...
                node_lon = celestial_data[
                    "node_longitude_deg"
                ]  # Mean Node from ephemeris service

                # Convert longitudes to Aubrey Hole numbers
                # Using the standard ASTRO_LONGITUDE_FOR_HOLE_0 from astronomical_calculations
                sun_hole = longitude_to_aubrey_hole(
                    sun_lon, NUM_HOLES, ASTRO_LONGITUDE_FOR_HOLE_0
                )
                moon_hole = longitude_to_aubrey_hole(
                    moon_lon, NUM_HOLES, ASTRO_LONGITUDE_FOR_HOLE_0
                )
                node_n_hole = longitude_to_aubrey_hole(
                    node_lon, NUM_HOLES, ASTRO_LONGITUDE_FOR_HOLE_0
                )
                ...
                self._ephemeris_seeded_initial_positions = {
                    ...
                    "M": moon_hole,
                    "N": node_n_hole,
                    "N'": self.node_n_prime_marker.current_position,
                }
                ...
                try:
                    ...
                    Exception
                ) as e_qdate:  # Catch any potential error during QDate creation itself
                    print(f"Warning: Could not create QDate for seed: {e_qdate}")
                    self._ephemeris_seed_gregorian_date = None

                print(
                    f"Simulation reset from ephemeris for {year}-{month:02d}-{day:02d} {hour:02d}:{minute:02d}:{second:02d} UTC."
                )
                print(
                    f"  Raw Longitudes: Sun={sun_lon:.2f}, Moon={moon_lon:.2f}, Node={node_lon:.2f}"
                )
                print(
                    f"  Converted Holes: Sun(S)={sun_hole}, Moon(M)={moon_hole}, Node(N)={node_n_hole}, Node(N')={self.node_n_prime_marker.current_position}"
                )
                print(
                    f"  Hoyle Time reset to: Day {self.time_state.current_day}, Year {self.time_state.current_year}"
                )
                ...
                if (
                    self.time_state.day_within_year_cycle == 1
                ):  # It will be due to TimeState(1,1)
                    if (
                        self.sun_marker.current_position
                        != self._summer_solstice_alignment_hole
                    ):
                        ...
                        print(
                            f"  Note: Ephemeris Sun at {sun_hole}. Standard Summer Solstice alignment is Hole {self._summer_solstice_alignment_hole}. Auto-correction will apply on next relevant cycle."
                        )

                return self.get_current_marker_positions()
            else:
                print(
                    f"Error: Could not retrieve ephemeris data for {year}-{month:02d}-{day:02d}."
                )
                return None
    ...
    def get_hoyle_prediction_for_date(self, target_year: int, target_day: int) -> dict:
        ...
        if (
            self.time_state.current_year == target_year
            and self.time_state.current_day == target_day
        ):
            ...
            if all(p != -1 for p in [s_pos, m_pos, n_pos, n_prime_pos]):
                ...
                if is_aligned_local(m_pos, s_pos, self.proximity_threshold) and (
                    self._is_in_nodal_zone(m_pos, n_pos)
                    or self._is_in_nodal_zone(m_pos, n_prime_pos)
                ):
                    hoyle_eclipses_on_date.append("Solar")
                # Check Hoyle Lunar Eclipse
                sun_opposite_pos_local = (s_pos + NUM_HOLES // 2) % NUM_HOLES
                if is_aligned_local(
                    m_pos, sun_opposite_pos_local, self.proximity_threshold
                ) and (
                    self._is_in_nodal_zone(m_pos, n_pos)
                    or self._is_in_nodal_zone(m_pos, n_prime_pos)
                ):
            ...
            if (
                self._ephemeris_seeded_initial_positions is not None
                and self._ephemeris_seed_gregorian_date
            ):
                try:
                    if isinstance(self._ephemeris_seed_gregorian_date, QDate):
                        results[
                            "sky_marker_positions_on_date"
                        ] = current_hoyle_marker_positions  # Same as Hoyle's current
                        results[
                            "equivalent_gregorian_date_str"
                        ] = self._ephemeris_seed_gregorian_date.toString("yyyy-MM-dd")

                        # Re-check eclipse conditions for sky (which are same as Hoyle's current here)
                        sky_eclipses = []
                        if all(
                            p != -1 for p in [s_pos, m_pos, n_pos, n_prime_pos]
                        ):  # Using already fetched s_pos etc.
                            if is_aligned_local(
                                m_pos, s_pos, self.proximity_threshold
                            ) and (
                                self._is_in_nodal_zone(m_pos, n_pos)
                                or self._is_in_nodal_zone(m_pos, n_prime_pos)
                            ):
                            ...
                            if is_aligned_local(
                                m_pos, sun_opposite_pos_sky, self.proximity_threshold
                            ) and (
                                self._is_in_nodal_zone(m_pos, n_pos)
                                or self._is_in_nodal_zone(m_pos, n_prime_pos)
                            ):
                        ...
                        results[
                            "sky_data_error"
                        ] = "Seed Gregorian date not in QDate format for Branch 1."
                        results["equivalent_gregorian_date_str"] = "Error"
                except (
                    Exception
                ) as e_sky_date:  # Catch any potential error during sky date processing
                    results[
                        "sky_data_error"
                    ] = f"Error processing sky date for Branch 1: {e_sky_date}"
                    results["equivalent_gregorian_date_str"] = "Error"
            else:  # Not seeded, or seed date info missing
                results[
                    "equivalent_gregorian_date_str"
                ] = "N/A (Not Seeded or Seed Date Missing)"
        ...
        temp_time_state = TimeState(
            start_day=1, start_year=1
        )  # Temp sim time always starts Y1,D1 for counting steps
        ...
        total_hoyle_days_from_calendar_start_to_target = (
            target_year - 1
        ) * DAYS_IN_HOYLE_YEAR + (target_day - 1)
        ...
        if self._ephemeris_seeded_initial_positions is not None:
            ...
            temp_sun_hoyle = Marker(
                name="S", initial_position=self._ephemeris_seeded_initial_positions["S"]
            )
            temp_moon_hoyle = Marker(
                name="M", initial_position=self._ephemeris_seeded_initial_positions["M"]
            )
            temp_node_n_hoyle = Marker(
                name="N", initial_position=self._ephemeris_seeded_initial_positions["N"]
            )
            temp_node_n_prime_hoyle = Marker(
                name="N'",
                initial_position=self._ephemeris_seeded_initial_positions["N'"],
            )
            ...
            hoyle_progression_time_state = TimeState(
                start_day=1, start_year=1
            )  # Corresponds to the seed date
            ...
            steps_for_hoyle_progression = (target_year - 1) * DAYS_IN_HOYLE_YEAR + (
                target_day - 1
            )
            ...
            for _ in range(steps_for_hoyle_progression):
                (
                    sun_cycle_comp,
                    year_cycle_comp,
                ) = hoyle_progression_time_state.advance_day()
                self._advance_hoyle_day_for_temp_simulation(
                    temp_sun_hoyle,
                    temp_moon_hoyle,
                    temp_node_n_hoyle,
                    temp_node_n_prime_hoyle,
            ...
            if is_aligned_local(m_pos_h, s_pos_h, self.proximity_threshold) and (
                self._is_in_nodal_zone(m_pos_h, n_pos_h)
                or self._is_in_nodal_zone(m_pos_h, n_prime_pos_h)
            ):
            ...
            hoyle_sim_results = {
                ...
                "hoyle_marker_positions_on_date": {
                    temp_sun_hoyle.name: s_pos_h,
                    temp_moon_hoyle.name: m_pos_h,
                    temp_node_n_hoyle.name: n_pos_h,
                    temp_node_n_prime_hoyle.name: n_prime_pos_h,
                },
            }
            ...
            if self._ephemeris_seed_gregorian_date:
                try:
                    ...
                    if isinstance(self._ephemeris_seed_gregorian_date, QDate):
                        ...
                        target_gregorian_date = (
                            self._ephemeris_seed_gregorian_date.addDays(
                                hoyle_days_elapsed_from_seed
                            )
                        )
                        equivalent_gregorian_date_str = target_gregorian_date.toString(
                            "yyyy-MM-dd"
                        )
                        ...
                        if sky_celestial_data:
                            sky_sun_hole = longitude_to_aubrey_hole(
                                sky_celestial_data["sun_longitude_deg"],
                                NUM_HOLES,
                                ASTRO_LONGITUDE_FOR_HOLE_0,
                            )
                            sky_moon_hole = longitude_to_aubrey_hole(
                                sky_celestial_data["moon_longitude_deg"],
                                NUM_HOLES,
                                ASTRO_LONGITUDE_FOR_HOLE_0,
                            )
                            sky_node_hole = longitude_to_aubrey_hole(
                                sky_celestial_data["node_longitude_deg"],
                                NUM_HOLES,
                                ASTRO_LONGITUDE_FOR_HOLE_0,
                            )
                            sky_node_prime_hole = (
                                sky_node_hole + NUM_HOLES // 2
                            ) % NUM_HOLES
                            ...
                            if is_aligned_local(
                                m_pos_sky, s_pos_sky, self.proximity_threshold
                            ) and (
                                self._is_in_nodal_zone(m_pos_sky, n_pos_sky)
                                or self._is_in_nodal_zone(m_pos_sky, n_prime_pos_sky)
                            ):
                                sky_eclipses.append("Solar")

                            sun_opp_sky = (s_pos_sky + NUM_HOLES // 2) % NUM_HOLES
                            if is_aligned_local(
                                m_pos_sky, sun_opp_sky, self.proximity_threshold
                            ) and (
                                self._is_in_nodal_zone(m_pos_sky, n_pos_sky)
                                or self._is_in_nodal_zone(m_pos_sky, n_prime_pos_sky)
                            ):
                            ...
                            sky_data_results = {
                                "error": "Failed to retrieve sky ephemeris data."
                            }
                    else:  # self._ephemeris_seed_gregorian_date is not a QDate instance
                        sky_data_results = {
                            "error": "Seed Gregorian date not in expected QDate format."
                        }
                except (
                    Exception
                ) as e:  # Broader catch for any other issues during sky data part
                    sky_data_results = {
                        "error": f"Error during sky data calculation: {str(e)}"
                    }
        ...
        # This branch only calculates Hoyle's model prediction.
            ...
            temp_node_n_prime_nominal = Marker(
                name="N'", initial_position=NUM_HOLES // 2
            )
            ...
            if is_aligned_local(m_pos_nom, s_pos_nom, self.proximity_threshold) and (
                self._is_in_nodal_zone(m_pos_nom, n_pos_nom)
                or self._is_in_nodal_zone(m_pos_nom, n_prime_pos_nom)
            ):
            ...
            hoyle_sim_results = (
                {  # Store nominal results under "hoyle_" keys for consistency
                    "hoyle_predicted_eclipses": nominal_eclipses,
                    "hoyle_marker_positions_on_date": {
                        temp_sun_nominal.name: s_pos_nom,
                        temp_moon_nominal.name: m_pos_nom,
                        temp_node_n_nominal.name: n_pos_nom,
                        temp_node_n_prime_nominal.name: n_prime_pos_nom,
                    },
                }
            )
            # No sky data for purely nominal Hoyle check unless explicitly requested by a different mechanism
            sky_data_results = {
                "equivalent_gregorian_date_str": "N/A (Nominal Hoyle Check)"
            }
        ...
        self.time_state.day_within_13_day_cycle = original_time_state_data[
            "day_within_13_day_cycle"
        ]
        self.time_state.day_within_year_cycle = original_time_state_data[
            "day_within_year_cycle"
        ]
