...
class TimeState:
    ...
    DAYS_IN_HOYLE_YEAR: int = (
        364  # 56 holes * 6.5 days/hole for Sun movement approximation
    )
    ...
    def __init__(self, start_day: int = 1, start_year: int = 1):
        ...
        self.day_within_13_day_cycle: int = (
            0  # Starts at 0, moves to 1 on first advance
        )
...
if __name__ == "__main__":
    ...
    for i in range(15):
        ...
        print(
            f"Day {i+1}: {time_state} -> Sun Cycle End: {sun_done}, Year Cycle End: {year_done}"
        )
    ...
    sun_done, year_done = time_state.advance_day()
    print(
        f"End of Year Test: {time_state} -> Sun Cycle End: {sun_done}, Year Cycle End: {year_done}"
    )
