...
if __name__ == '__main__':
    ...
    print(f"Sample Eclipse Event: {sample_eclipse.year}-{sample_eclipse.month:02d}-{sample_eclipse.day:02d}")
    print(f"  Type: {sample_eclipse.eclipse_type}, Catalog: {sample_eclipse.cat_no}")
    print(f"  Magnitude: {sample_eclipse.magnitude}")
    print(f"  Greatest Eclipse at: Lat {sample_eclipse.lat_dd_ge}, Lng {sample_eclipse.lng_dd_ge}")
    print(f"  Central Duration: {sample_eclipse.central_duration}, Path Width: {sample_eclipse.path_width}km")
    print(f"  Time of Greatest Eclipse: {sample_eclipse.td_ge}")

    try:
        invalid_data = EclipseData(year="not_a_year", month=1, day=1, eclipse_type="T", cat_no="test")
