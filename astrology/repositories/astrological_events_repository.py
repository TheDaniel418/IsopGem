...
class AstrologicalEventsRepository:
    ...
    def _initialize_celestial_bodies(self) -> None:
        ...
        try:
                ...
                for name, body_type in celestial_bodies:
                    if name not in existing_bodies:
                        ...
                        try:
                            ...
                            logger.error(
                                f"SQLite error inserting celestial body {name}: {e}"
                            )
                        except Exception as e:
            ...
            count = self.database.query_one(
                "SELECT COUNT(*) as count FROM celestial_bodies"
            )
            logger.info(
                f"Celestial bodies initialized: {count['count']} bodies in database"
            )
    ...
    def get_aspects(
        ...
        for row in rows:
            ...
            if aspect["applying_timestamp"]:
                aspect["applying_timestamp"] = datetime.fromisoformat(
                    aspect["applying_timestamp"]
                )
            if aspect["exact_timestamp"]:
                aspect["exact_timestamp"] = datetime.fromisoformat(
                    aspect["exact_timestamp"]
                )
            if aspect["separation_timestamp"]:
                aspect["separation_timestamp"] = datetime.fromisoformat(
                    aspect["separation_timestamp"]
                )
    ...
    def verify_aspects_in_database(self, year: int = None) -> Dict:
        ...
        try:
            ...
            # Log the results
            logger.info(
                f"Aspect verification results: {results['total_aspects']} total aspects found"
            )
            if year is not None:
                logger.info(
                    f"Aspects for year {year}: {results['aspects_by_year'].get(year, 0)}"
                )
            ...
            if results["total_aspects"] == 0:
                ...
                logger.warning(
                    "No aspects found in database. Calculation may have failed."
                )
            ...
            return {"total_aspects": 0, "status": "error", "error": str(e)}
    ...
    def get_lunar_phases(
        self, start_date=None, end_date=None, year=None, phase_type=None
    ):
    ...
    @handle_database_error
    def get_planet_phases(
        self, start_date=None, end_date=None, year=None, body_name=None, phase_type=None
    ):
    ...
    def get_available_date_range(self) -> Tuple[Optional[int], Optional[int]]:
        ...
        try:
            ...
            min_year = (
                min_result["min_year"]
                if min_result and min_result["min_year"] is not None
                else None
            )
            max_year = (
                max_result["max_year"]
                if max_result and max_result["max_year"] is not None
                else None
            )
            ...
            if min_year is None or max_year is None:
                ...
                if (
                    year_result
                    and "min_year" in year_result
                    and "max_year" in year_result
                ):
                    min_year = (
                        year_result["min_year"]
                        if min_year is None
                        else min(min_year, year_result["min_year"])
                    )
                    max_year = (
                        year_result["max_year"]
                        if max_year is None
                        else max(max_year, year_result["max_year"])
                    )
