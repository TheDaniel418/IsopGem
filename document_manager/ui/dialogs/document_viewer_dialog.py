"""
Purpose: Provides a dialog for viewing document content and details.
...
class DocumentViewerDialog(QDialog):
    ...
    def _init_ui(self):
        ...
        title_label = QLabel(self.document.name if self.document else "Document Not Found")
        title_font = QFont()
        ...
        if self.document.content:
            ...
            if self.document_service.extract_text(self.document):
                ...
                if updated_doc and updated_doc.content:
                    ...
                    self.text_content.setText("No text content available for this document.")
            else:
                self.text_content.setText("Failed to extract text from this document type.")
    ...
    def _load_metadata(self) -> None:
        ...
        self._add_metadata_item("Type", self.document.file_type.value.upper(), "File Information")
        self._add_metadata_item("Size", self.document.get_file_size_display(), "File Information")
        self._add_metadata_item("Path", str(self.document.file_path), "File Information")
        ...
        if self.document.last_modified_date:
            modified_date = self.document.last_modified_date.strftime("%Y-%m-%d %H:%M:%S")
        ...
        if self.document.page_count:
            self._add_metadata_item("Pages", str(self.document.page_count), "Document Details")
        
        if self.document.word_count:
            self._add_metadata_item("Words", str(self.document.word_count), "Document Details")
        ...
        if self.document.metadata:
            for key, value in self.document.metadata.items():
                if value is not None:
                    self._add_metadata_item(key.capitalize(), str(value), "Additional Metadata")
    
    def _add_metadata_item(self, name: str, value: Optional[Any], parent_name: Optional[str] = None) -> None:
        ...
        if parent_name:
            ...
            parent_items = self.metadata_tree.findItems(parent_name, Qt.MatchFlag.MatchExactly, 0)
