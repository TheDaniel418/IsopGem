...
class CategoryManagerDialog(QDialog):
    ...
    def _build_tree_items(
        self, categories: List[Dict], parent_item: Optional[QTreeWidgetItem] = None
    ):
        ...
        for category in categories:
            ...
            item.setForeground(
                2, self._contrasting_text_color(QColor(category["color"]))
            )
    ...
    def _show_context_menu(self, position):
        ...
        add_child_action.triggered.connect(
            lambda: self._add_child_category(category_id)
        )
    ...
    def _add_child_category(self, parent_id: Optional[str] = None):
        ...
        if parent_id is None:
            ...
            if not selected_items:
                MessageBox.warning(
                    self, "No Selection", "Please select a parent category first."
                )
                return
        ...
        if not parent:
            MessageBox.error(
                self, "Invalid Parent", "The selected parent category is not valid."
            )
            return
    ...
    def _create_category(self, parent_id: Optional[str]):
        ...
        description, ok = QInputDialog.getMultiLineText(
            self, "Category Description", "Enter category description (optional):"
        )
    ...
    def _edit_category(self, category_id: Optional[str] = None):
        ...
        if not category:
            MessageBox.error(
                self, "Category Not Found", "The selected category could not be found."
            )
            return

        # Get updated name
        name, ok = QInputDialog.getText(
            self, "Edit Category", "Category name:", text=category.name
        )
        ...
        color = QColorDialog.getColor(
            QColor(category.color), self, "Select Category Color"
        )
        if not color.isValid():
        ...
        updated_category = self.category_service.update_category(
            category_id=category_id, **category_data
        )
