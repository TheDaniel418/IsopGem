...
class DocumentTab(QWidget):
    ...
    def _init_ui(self) -> None:
        ...
        if not seshat_pixmap.isNull():
            ...
            # Add caption
            caption = QLabel("Seshat - Egyptian goddess of writing, record keeping, and archives")
            caption.setStyleSheet("font-size: 12px; color: #795548; font-style: italic; background-color: rgba(255, 255, 235, 0.8); padding: 4px; border-radius: 4px;")
            caption.setAlignment(Qt.AlignmentFlag.AlignCenter)
            
            seshat_layout.addWidget(seshat_image_label, alignment=Qt.AlignmentFlag.AlignCenter)
            seshat_layout.addWidget(caption, alignment=Qt.AlignmentFlag.AlignCenter)
            
            # Add the image container to the content layout
            content_layout.addWidget(seshat_container, alignment=Qt.AlignmentFlag.AlignCenter)
        else:
            logger.error("Failed to load Seshat image from assets/tab_images/sechat.png")
