...
from document_manager.ui.dialogs.concordance_creation_dialog import ConcordanceCreationDialog
...
class ConcordancePanel(QWidget):
    ...
    def _setup_ui(self):
        ...
        self.create_button.setStyleSheet("QPushButton { background-color: #3498db; color: white; padding: 8px 16px; border: none; border-radius: 4px; }")
    ...
    def _load_concordance_tables(self):
        ...
        try:
            ...
            QMessageBox.warning(self, "Error", f"Failed to load concordance tables: {str(e)}")
    ...
    def _populate_tables_list(self):
        ...
        for table in self.concordance_tables:
            if (not search_text or 
                search_text in table['name'].lower() or
                (table['description'] and search_text in table['description'].lower()) or
                any(search_text in tag.lower() for tag in table.get('tags', []))):
    ...
    def _load_concordance_table(self, table_id: str):
        ...
        try:
            self.current_concordance = self.concordance_service.get_concordance(table_id)
    ...
    def _populate_document_filter(self):
        ...
        if self.current_concordance:
            ...
            doc_names = set(entry.document_name for entry in self.current_concordance.entries)
    ...
    def _display_concordance_entries(self):
        ...
        self.results_label.setText(f"Showing {filtered_count} of {total_entries} entries")
        ...
        for row, entry in enumerate(filtered_entries):
            ...
            # Line
            line_item = QTableWidgetItem(str(entry.line_number) if entry.line_number else "")
    ...
    def _on_entry_selection_changed(self):
        ...
        if selected_items:
            ...
            if entry:
                ...
                full_context = f"{entry.left_context} **{entry.keyword}** {entry.right_context}"
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                if self.concordance_service.delete_concordance(table_id):
                    self._load_concordance_tables()
                    QMessageBox.information(self, "Success", "Concordance table deleted successfully")
                else:
                    QMessageBox.warning(self, "Error", "Failed to delete concordance table")
            except Exception as e:
                QMessageBox.warning(self, "Error", f"Failed to delete concordance: {str(e)}")
    ...
    def _export_selected_table(self):
        ...
        filename, _ = QFileDialog.getSaveFileName(
            self,
            "Export Concordance",
            f"{self.current_concordance.name}.csv",
            "CSV Files (*.csv);;TSV Files (*.tsv);;JSON Files (*.json);;HTML Files (*.html);;Text Files (*.txt)"
        )
        ...
        if filename:
            try:
                ...
                QMessageBox.information(self, "Success", f"Concordance exported to {filename}")
                
            except Exception as e:
                QMessageBox.warning(self, "Error", f"Failed to export concordance: {str(e)}") 